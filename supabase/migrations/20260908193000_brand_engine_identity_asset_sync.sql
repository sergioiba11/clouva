-- Synchronize the production Brand Engine schema with the current resolver.
-- This migration is intentionally additive/idempotent because older environments
-- may already have the 20260806140000 real-import migration while production
-- historically skipped it.
...
alter table public.brand_clearance_checks add constraint brand_clearance_checks_manual_decision_check
  check (manual_decision is null or manual_decision in ('clear','review_required','blocked_external_name_conflict','blocked_external_visual_conflict','blocked_combined_conflict'));
...
declare
  ...
  v_is_generated_or_reconstructed boolean;
...
begin
  ...
  -- A manual, real identity import is allowed to become official without an AI
  -- regeneration/clearance cycle. Generated/reconstructed identities retain the
  -- stricter professional-master and clearance requirements.
  v_is_generated_or_reconstructed := v_import_mode in ('owned_identity_reconstruction','clouva_generated_redesign','standalone_creation');
  ...
  if v_is_generated_or_reconstructed and v_master_svg_url is null then
  ...
  if v_is_generated_or_reconstructed and v_clearance_status is distinct from 'clear' then
