...
import { fallbackPublicStudioReference } from "./lib/server/public-studio-reference.ts";
...
test("post-login destinations keep Home separate from Mi Flow", () => {
  ...
  const existingUser = { created_at: "2026-08-01T10:00:00.000Z", last_sign_in_at: "2026-08-31T20:00:00.000Z" };
  const newUser = { created_at: "2026-08-31T20:00:00.000Z", last_sign_in_at: "2026-08-31T20:00:05.000Z" };
...
test("Player destination has exactly three lifecycle outcomes", () => {
  ...
  assert.equal(getPlayerDestination({ slug: "clouva", is_published: false, publication_status: "draft" }), "/profile/edit");
  assert.equal(getPlayerDestination({ slug: "clouva", is_published: true, publication_status: "published" }), "/clouva");
  assert.equal(getPlayerDestination({ slug: "Bless Music", is_published: true, publication_status: "published" }), "/Bless%20Music");
});
...
test("master navigation contract is shared and keeps product concepts separate", () => {
  ...
  assert.equal(CLOUVA_NAVIGATION.MATRIX.href, "/lamatrix");
  assert.equal(CLOUVA_NAVIGATION.STUDIOS.href, "/lamatrix/estudios");
...
test("legacy aliases are real redirects and canonical UIs do not generate product aliases", () => {
...
test("every existing root system route is reserved from public Player aliases", () => {
  const required = [
    "account", "admin", "api", "auth", "avatar-analyzer-v4", "biblioteca", "carrito", "catalogo", "checkout", "clouva-ai", "crear", "creator-studio", "cuenta", "debug-auth", "empleado", "gracias", "lamatrix", "login", "logo", "lookbook", "matrix", "mi-flow", "mi-qr", "mi-spot", "onboarding", "pedido", "perfil", "perfil-publico", "players", "privacidad", "producto", "profile", "q", "registro", "shop", "sobre-clouva", "spaces", "studio-dashboard", "studios", "terminos", "tienda", "truco", "u", "vip",
  ];
...
test("canonical Studio identity separates internal slug from public identity", () => {
  const fallback = fallbackPublicStudioReference({ id: "studio-1", slug: "internal-studio", name: "Internal Studio", share_title: "PUBLIC RECORDS", logo_url: "https://example.com/logo.png" });
  assert.equal(fallback.publicAlias, "internal-studio");
  assert.equal(fallback.publicName, "PUBLIC RECORDS");
  assert.equal(fallback.href, "/lamatrix/estudios/internal-studio");
  const resolver = read("./lib/server/public-studio-reference.ts");
  assert.match(resolver, /\.eq\("is_primary"|find\(\(row\) => row\.is_primary\)/);
  assert.match(resolver, /share_title/);
  assert.match(resolver, /\/api\/brand-assets\/\$\{encodeURIComponent\(activeBrandVersionId\)\}\/logo\?surface=dark/);
});

test("Matrix and Studio legacy surfaces permanently converge on canonical routes", () => {
  assert.match(read("./app/matrix/page.tsx"), /permanentRedirect\("\/lamatrix"\)/);
  assert.match(read("./app/studios/page.tsx"), /permanentRedirect\("\/lamatrix\/estudios"\)/);
  const legacyStudio = read("./app/studios/[slug]/page.tsx");
  assert.match(legacyStudio, /permanentRedirect/);
  assert.match(legacyStudio, /studioPublicHref/);
  const sitemap = read("./app/sitemap.ts");
  assert.match(sitemap, /"\/lamatrix"/);
  assert.match(sitemap, /"\/lamatrix\/estudios"/);
  assert.doesNotMatch(sitemap, /staticRoutes = \[[^\]]*"\/matrix"/);
});

test("Player Studio links are canonical, public-named and Brand-Asset aware", () => {
  const standard = read("./components/public/PlayerPublicView.tsx");
  const precise = read("./components/public/PrecisePlayerLayoutRenderer.tsx");
  for (const source of [standard, precise]) {
    assert.match(source, /public_name/);
    assert.match(source, /official_logo_url/);
    assert.match(source, /public_href/);
    assert.doesNotMatch(source, /href=\{`\/studios\/\$\{[^}]+\.slug\}/);
  }
  assert.match(precise, /mobileFontCap/);
  assert.match(precise, /overflow-x-hidden/);
  assert.match(precise, /mobileMinHeight/);
});
...
test("Crear is a hub over real existing tools and Media Creator lives below it", () => {
  ...
  for (const href of ["/crear/media", "/clouva-ai", "/creator-studio", "/mi-flow/avatar", "/mi-flow/crear-prenda", "/mi-flow/creative"]) assert.match(createHub, new RegExp(href.replaceAll("/", "\\/")));
...
test("AccountMenu is personal, compact and admin-gated", () => {
  ...
  for (const label of ["MI FLOW", "MI SPOT", "MI PLAYER / PERFIL PÚBLICO", "MI QR", "CONFIGURACIÓN", "TODO CLOUVA", "MIS ESTUDIOS", "CAMBIAR CUENTA", "CERRAR SESIÓN"]) assert.match(menu, new RegExp(label.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
