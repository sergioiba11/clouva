...
import type { ClouvaAIContentPart } from "./providers/types";
...
export function normalizeScreenContext(value: unknown): Record<string, unknown> {
  ...
  catch { throw new Error("El contexto de Workspace no es serializable."); }
  if (Buffer.byteLength(serialized, "utf8") > 24_000) throw new Error("El contexto de Workspace es demasiado grande.");
  ...
  return {
    ...
    project: { ...context.project, id: cleanText(legacyProject.id, 160), path: cleanText(legacyProject.path, 1_000) },
    preview: {
      url: cleanUrl(legacyPreview.url),
      state: cleanText(legacyPreview.state, 80),
      route: cleanText(legacyPreview.route, 300),
      viewport: {
        width: typeof legacyViewport.width === "number" && Number.isFinite(legacyViewport.width) ? Math.max(0, Math.round(legacyViewport.width)) : undefined,
        height: typeof legacyViewport.height === "number" && Number.isFinite(legacyViewport.height) ? Math.max(0, Math.round(legacyViewport.height)) : undefined,
      },
    },
  };
}
...
export function normalizeAttachments(value: unknown): NormalizedAttachment[] {
  ...
  if (!Array.isArray(value) || value.length > MAX_ATTACHMENTS) throw new Error(`Se permiten hasta ${MAX_ATTACHMENTS} adjuntos por mensaje.`);
  let totalBytes = 0;
  return value.map((raw, index) => {
    ...
    const kind = ["image", "audio", "file", "preview"].includes(String(raw.kind)) ? raw.kind as NormalizedAttachment["kind"] : "file";
    ...
    if (!bytes.length || bytes.length > MAX_ATTACHMENT_BYTES) throw new Error(`El adjunto “${name}” supera el máximo de 5 MB o está vacío.`);
...
export function attachmentContentPart(attachment: NormalizedAttachment): ClouvaAIContentPart {
  if (attachment.mimeType.startsWith("text/") || attachment.mimeType === "application/json") {
    ...
    return { type: "text", text: `ARCHIVO ADJUNTO: ${attachment.name}\n\n${text.slice(0, 120_000)}` };
  }
  return {
    type: "media",
...
/** @deprecated Compatibility helper for legacy Gemini-only code. */
export function attachmentPart(attachment: NormalizedAttachment): Record<string, unknown> {
  const part = attachmentContentPart(attachment);
