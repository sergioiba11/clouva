...
export async function listReferenceAssets(): Promise<ReferenceAsset[]> {
  ...
  const { data, error } = await supabase.from(TABLE).select("id,user_id,name,category,file_name,file_size,storage_path,source_url,license,author,created_at").eq("user_id", userId).order("created_at", { ascending: false });
  if (error) throw new Error(`No se pudo leer la biblioteca online: ${error.message}`);

  const results = await Promise.allSettled((data ?? []).map(async (row: any) => {
    ...
    return {
      id: row.id, ownerId: row.user_id, name: row.name, category: row.category as ReferenceCategory,
      fileName: row.file_name, size: Number(row.file_size ?? blob.size), createdAt: new Date(row.created_at).getTime(),
      sourceUrl: row.source_url ?? undefined, license: row.license ?? undefined, author: row.author ?? undefined,
      storagePath: row.storage_path, file: blob,
...
export async function saveReferenceAsset(asset: ReferenceAsset): Promise<void> {
  ...
  const { error: insertError } = await supabase.from(TABLE).insert({ id: asset.id, user_id: userId, name: asset.name, category: asset.category, file_name: asset.fileName, file_size: asset.size, storage_path: storagePath, source_url: asset.sourceUrl ?? null, license: asset.license ?? null, author: asset.author ?? null });
  if (insertError) {
...
export function makeReferenceAsset(file: File, category: ReferenceCategory, metadata: Partial<Pick<ReferenceAsset, "name" | "sourceUrl" | "license" | "author">> = {}): ReferenceAsset {
  return { id: crypto.randomUUID(), name: metadata.name?.trim() || file.name.replace(/\.glb$/i, ""), category, fileName: file.name, size: file.size, createdAt: Date.now(), sourceUrl: metadata.sourceUrl?.trim() || undefined, license: metadata.license?.trim() || undefined, author: metadata.author?.trim() || undefined, file };
}
