...
function sanitizeFileName(value: string) {
  return value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-zA-Z0-9._-]+/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "") || "model.glb";
}

function isTemplateStatus(status: unknown): boolean {
...
async function rowToAsset(row: any): Promise<ReferenceAsset> {
  const status = String(row.status ?? "reference") as ReferenceAssetStatus;
  const isTemplate = isTemplateStatus(status);
  const selectedStoragePath = isTemplate && row.rigged_storage_path
    ? row.rigged_storage_path
    : row.storage_path;
  ...
  return {
    ...
    riggedStoragePath: row.rigged_storage_path ?? undefined,
    previewSettings: row.preview_settings && typeof row.preview_settings === "object" ? row.preview_settings : {},
    status,
    isTemplate,
    file: blob,
  };
...
export async function listReferenceAssets(): Promise<ReferenceAsset[]> {
  ...
  const { data, error } = await supabase.from(TABLE).select(SELECT_FIELDS).eq("user_id", userId).order("created_at", { ascending: false });
  ...
  return results
    ...
    .sort((a, b) => Number(b.isTemplate) - Number(a.isTemplate) || b.createdAt - a.createdAt);
}

export async function getReferenceAssetById(id: string): Promise<ReferenceAsset | null> {
  const userId = await requireUserId();
  const { data, error } = await supabase.from(TABLE).select(SELECT_FIELDS).eq("id", id).eq("user_id", userId).maybeSingle();
...
export async function saveReferenceAsset(asset: ReferenceAsset): Promise<void> {
  ...
  const { error: uploadError } = await supabase.storage.from(BUCKET).upload(storagePath, asset.file, { contentType: "model/gltf-binary", cacheControl: "3600", upsert: false });
...
export async function promoteReferenceAssetToTemplate(id: string, previewSettings: Record<string, unknown>): Promise<void> {
  const userId = await requireUserId();
  const { data, error } = await supabase.from(TABLE).select("storage_path").eq("id", id).eq("user_id", userId).maybeSingle();
  if (error) throw new Error(`No se pudo localizar el GLB: ${error.message}`);
  if (!data?.storage_path) throw new Error("El GLB no tiene una ruta de Storage válida.");

  const { error: updateError } = await supabase.from(TABLE).update({
    status: "ready",
    rigged_storage_path: data.storage_path,
    preview_settings: previewSettings,
    updated_at: new Date().toISOString(),
  }).eq("id", id).eq("user_id", userId);
  if (updateError) throw new Error(`No se pudo convertir el GLB en plantilla: ${updateError.message}`);
}

export async function setReferenceAssetProcessing(id: string, previewSettings: Record<string, unknown>): Promise<void> {
  const userId = await requireUserId();
  ...
  if (error) throw new Error(`No se pudo actualizar el estado del GLB: ${error.message}`);
}
...
export async function saveRiggedReferenceAsset(
  ...
  const userId = await requireUserId();
  const { data: row, error: rowError } = await supabase.from(TABLE).select(SELECT_FIELDS).eq("id", id).eq("user_id", userId).maybeSingle();
  if (rowError) throw new Error(`No se pudo localizar el GLB: ${rowError.message}`);
  if (!row) throw new Error("El GLB original ya no existe en la biblioteca.");
  ...
  const riggedName = `rigged-${sanitizeFileName(row.file_name)}`;
  const riggedStoragePath = `${userId}/${id}/${riggedName}`;
  const riggedBlob = new Blob([bytes], { type: "model/gltf-binary" });
  const { error: uploadError } = await supabase.storage.from(BUCKET).upload(riggedStoragePath, riggedBlob, {
  ...
  const { error: updateError } = await supabase.from(TABLE).update({
    status: "ready",
    rigged_storage_path: riggedStoragePath,
    preview_settings: previewSettings,
    updated_at: new Date().toISOString(),
  }).eq("id", id).eq("user_id", userId);
  if (updateError) {
    await supabase.storage.from(BUCKET).remove([riggedStoragePath]);
    throw new Error(`El GLB se riggeó, pero no se pudo actualizar la biblioteca: ${updateError.message}`);
  }
...
export async function markReferenceAssetError(id: string): Promise<void> {
  const userId = await requireUserId();
  await supabase.from(TABLE).update({ status: "error", updated_at: new Date().toISOString() }).eq("id", id).eq("user_id", userId);
}

export async function deleteReferenceAsset(id: string): Promise<void> {
  const userId = await requireUserId();
  const { data, error } = await supabase.from(TABLE).select("storage_path,rigged_storage_path").eq("id", id).eq("user_id", userId).maybeSingle();
  if (error) throw new Error(`No se pudo localizar el GLB: ${error.message}`);
  if (!data) return;
  const paths = [data.storage_path, data.rigged_storage_path].filter((value): value is string => Boolean(value));
