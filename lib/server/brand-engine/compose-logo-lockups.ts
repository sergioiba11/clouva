...
function textToPath(text: string, fontSizePx: number, trackingPx: number) {
  ...
  for (const char of Array.from(text)) {
    if (char === " ") { x += fontSizePx * 0.4 + trackingPx; continue; }
...
function applyCase(text: string, uppercase: boolean) { return uppercase ? text.toLocaleUpperCase("es-AR") : text; }
...
function innerSvg(svg: string) { return svg.replace(/^.*?<svg[^>]*>/is, "").replace(/<\/svg>\s*$/is, ""); }
...
function recolorSvg(svg: string, color: string) { return svg.replace(/<svg([^>]*)>/i, `<svg$1><style>path,rect,circle,ellipse,polygon,polyline,line{fill:${color}!important;stroke:${color}!important}</style>`); }
...
function buildTextPaths(args: { naming: BrandNaming; typography: TypographyConfig }) {
  ...
  return { name: textToPath(name, 96, args.typography.primaryTracking), descriptor: descriptor ? textToPath(descriptor, 96 * args.typography.descriptorScale, args.typography.descriptorTracking) : null };
}
...
function buildStackedSvg(symbolSvg: string, naming: BrandNaming, typography: TypographyConfig) {
  ...
  const padding = 44, symbolSize = 260, gap = 28;
  ...
  const symbolX = (width - symbolSize) / 2, nameX = (width - paths.name.width) / 2, nameY = padding + symbolSize + gap + paths.name.height * 0.8;
...
function buildHorizontalSvg(symbolSvg: string, naming: BrandNaming, typography: TypographyConfig) {
  const paths = buildTextPaths({ naming, typography });
  const padding = 40, symbolSize = 220, gap = 34;
  ...
  const textX = padding + symbolSize + gap, nameY = padding + (Math.max(symbolSize, textHeight) - textHeight) / 2 + paths.name.height * 0.8;
...
function buildSquareSvg(symbolSvg: string, naming: BrandNaming, typography: TypographyConfig) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 1000">${nestedSvg(symbolSvg, 230, 120, 540, 540)}<g transform="translate(500 760) scale(${nameScale}) translate(${-paths.name.width / 2} 0)"><path data-component="wordmark" d="${paths.name.d}" fill="#ffffff"/></g></svg>`;
}

function generatedSymbolDetectedLogo(): DetectedLogo {
  return {
    detected: true, confidence: 1, primaryBox: { left: 0, top: 0, right: 1000, bottom: 1000 }, occurrences: [], logoType: "symbol",
    ...
    decomposition: { components: [{ kind: "full_lockup", present: true, confidence: 1, box: { left: 0, top: 0, right: 1000, bottom: 1000 }, description: "Símbolo aislado", expectedText: null }], foregroundPolarity: "mixed", recommendedColorCount: 4, backgroundDescription: "fondo de generación" },
  };
}

export type LogoLockupSvgSet = { master: string; symbol: string; primary: string; horizontal: string; vertical: string; square: string; white: string; black: string; monochrome: string; favicon: string };

export async function composeLogoLockupSvgs(args: { masterSymbolBytes: Buffer; naming: BrandNaming; typography: TypographyConfig; lockupStructure: LogoLockupStructure | null }): Promise<LogoLockupSvgSet> {
  const symbolVector = await reconstructLogoVector({ referenceBytes: args.masterSymbolBytes, detectedLogo: generatedSymbolDetectedLogo(), params: { colorCount: 4, paddingPct: 0, backgroundTolerance: 28, localContrastThreshold: 7, minComponentArea: 10, simplifyTolerance: 1.2 } });
  ...
  return { master: primary, symbol, primary, horizontal, vertical: stacked, square, white: recolorSvg(primary, "#ffffff"), black: recolorSvg(primary, "#000000"), monochrome: recolorSvg(primary, "#000000"), favicon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">${nestedSvg(symbol, 40, 40, 432, 432)}</svg>` };
}
...
export async function composeLogoLockups(args: { masterSymbolBytes: Buffer; naming: BrandNaming; typography: TypographyConfig; lockupStructure: LogoLockupStructure | null }): Promise<LogoCandidateVariants> {
  ...
  const [primary, symbol, horizontal, vertical, square, transparent, white, black, favicon] = await Promise.all([
    render(svgs.primary, 1400, 1000, "#0a0a0a"), render(svgs.symbol, 1024, 1024, "#0a0a0a"), render(svgs.horizontal, 1600, 600, "#0a0a0a"), render(svgs.vertical, 1000, 1200, "#0a0a0a"), render(svgs.square, 1024, 1024, "#0a0a0a"), render(svgs.primary, 1600, 1200), render(svgs.white, 1600, 1200), render(svgs.black, 1600, 1200), render(svgs.favicon, 512, 512),
  ]);
  const cleanTransparent = await removeBackground(transparent);
  return {
    primary: { bytes: primary, mimeType: "image/png" }, symbol: { bytes: await toSquare(symbol, 1024), mimeType: "image/png" }, horizontal: { bytes: horizontal, mimeType: "image/png" }, vertical: { bytes: vertical, mimeType: "image/png" }, square: { bytes: square, mimeType: "image/png" }, transparent: { bytes: cleanTransparent, mimeType: "image/png" }, white: { bytes: await flattenToColor(cleanTransparent, [255, 255, 255]), mimeType: "image/png" }, black: { bytes: await flattenToColor(cleanTransparent, [0, 0, 0]), mimeType: "image/png" }, favicon: { bytes: favicon, mimeType: "image/png" },
  };
}
