...
export async function generateLayoutConfig(args: {
  ...
  const promptText = [
    ...
    "VARIANTES PERMITIDAS POR SECCIÓN:",
    ...
    "- \"music\" solo si hay pistas concretas de Spotify/YouTube en los datos -- si no hay, no la incluyas.",
    ...
    '  "sections": [ { "type": string, "variant": string, "headline": string, "subheadline": string, "heading": string, "body": string, "items": [{"title": string, "description": string}], "embeds": [{"url": string}] } ],',
    '  "page_style": { "theme": "dark" | "light" | "mixed", "palette": { "background": string, "surface": string, "text": string, "muted_text": string, "accent": string, "border": string } },',
