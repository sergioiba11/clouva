...
def noisy_open_garment_mesh() -> trimesh.Trimesh:
    mesh = open_garment_mesh()
    keep = np.ones(len(mesh.faces), dtype=bool)
    # punch multiple tiny holes to simulate the noisy loop situation seen in r1.
    for idx in [3, 7, 12, 18, 24, 33, 44, 51, 57, 63, 71, 79]:
        if idx < len(keep):
            keep[idx] = False
...
def main():
    with tempfile.TemporaryDirectory() as temp:
        root = Path(temp)
        garment = noisy_open_garment_mesh()
        ...
        assert alignment["method"] == "body-frame-loop-gated-weighted-translation-v1.1.3"
        assert alignment["boundary_mode"] in {"fallback_top_center", "boundary_loop"}
        ...
        assert fitted.fit_json["version"] == "clouva-template-fit-engine-v1.1.3"
        assert fitted.fit_json["alignment"]["translation_strategy"]["torso_weight"] >= 0.20
    print("V113_LOOP_GATING_ALIGNMENT_OK")
