...
test("Trébol conserva la conversación al abrirse y sólo crea otra por acción explícita", () => {
  const full = read("./components/clouva-ai/ClouvaAIChat.tsx");
  const provider = read("./components/clouva-ai/ClouvaAIAssistantProvider.tsx");
  assert.match(full, /if \(sharedConversationId\)/);
  assert.match(full, /\.eq\("id", sharedConversationId\)/);
  assert.match(full, /async function loadLatestConversation\(\)/);
  assert.match(full, /function newConversation\(\)[\s\S]*?updateConversationId\(null\)/);
  assert.match(provider, /setConversationId/);
});
