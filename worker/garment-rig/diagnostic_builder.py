...
def _marker(name: str, item: dict, radius: float):
    bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=2, radius=radius, location=_point(item))
...
def build_diagnostic_glb(output_path: Path, avatar_meshes: Iterable[bpy.types.Object],
    ...
    for start_name, end_name in [*BODY_EDGES, *FACE_EDGES, *_finger_edges(landmarks)]:
        if start_name not in landmarks or end_name not in landmarks:
            continue
        diagnostic_objects.append(_edge(
            f"EDGE_{start_name}__{end_name}",
            _point(landmarks[start_name]),
            _point(landmarks[end_name]),
            edge_radius,
        ))
    ...
    return {
        ...
        "layers": sorted({_layer(name) for name in landmarks}),
    }
