"""Project MediaPipe 2D candidates onto the actual Blender mesh surface."""
...
def _perspective_ray(camera: bpy.types.Object, x: float, y: float, resolution):
    ...
    tan_y = __import__("math").tan(float(camera.data.angle_y) * 0.5)
    tan_x = tan_y * aspect
    local = Vector((ndc_x * tan_x, ndc_y * tan_y, -1.0)).normalized()
...
def _cast_allowed(origin: Vector, direction: Vector, classifications: Dict[str, str],
                  allowed_classes: set, max_distance: float = 10000.0):
    ...
    for view_result in detector_output.get("views", []):
        ...
        for candidate in view_result.get("candidates", []):
            if camera.data.type == "ORTHO":
                origin, direction = _orthographic_ray(
                    camera, candidate["x"], candidate["y"], view.get("resolution", [512, 512])
                )
            else:
                origin, direction = _perspective_ray(
                    camera, candidate["x"], candidate["y"], view.get("resolution", [512, 512])
                )
            name = str(candidate.get("name") or "")
            allowed = {"body", "unknown"}
            if name.startswith("eye_"):
                allowed.add("eyes")
            hit, rejected = _cast_allowed(origin, direction, classifications, allowed)
            if hit is None:
                failures.append({
                    "code": "LANDMARK_RAY_MISS",
                    "name": name,
                    "view": view_name,
                    "rejected": rejected,
                })
            ...
            projected.append({
                **candidate,
                ...
                "geometryConfidence": 0.92 if not rejected else 0.72,
            })
