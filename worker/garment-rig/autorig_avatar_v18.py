"""Analyzer-gated wrapper around the existing CLOUVA AutoRig V16.

AutoRig V16 remains the production armature and weight implementation. This
wrapper canonicalizes the same clean input as Avatar Analyzer V3.2 and uses only
approved body landmarks as Skeleton Planner seeds. Missing or rejected points
fall back to V16 geometry detection; no point is invented.
"""
...
EXPECTED_ANALYZER_VERSION = "clouva-avatar-analyzer-v3.2"


def _analysis_path():
    value = os.environ.get(ANALYSIS_ENV, "").strip()
    if not value:
        raise RuntimeError("AutoRig requires the approved Avatar Analyzer map")
    path = Path(value).resolve()
    if not path.is_file():
        raise RuntimeError("Avatar Analyzer map file is missing")
    return path
...
def _load_analysis(input_path: Path):
    ...
    if analysis.get("status") not in {"valid", "valid_with_warnings"}:
        raise RuntimeError(f"Avatar Analyzer did not approve the source: {analysis.get('status')}")
    if analysis.get("rigReadinessApproved") is not True:
        raise RuntimeError(f"Rig readiness is blocked: {analysis.get('rigReadinessGates')}")
    ...
    if analysis.get("criticalLandmarksVerified") is not True:
        raise RuntimeError("Critical body landmarks were not verified")
    return path, analysis


def _point(record):
    if not isinstance(record, dict) or not record.get("accepted", False):
        return None
...
def _seed_detector(analysis: dict, seeded: list[str]):
    ...
    records = analysis.get("landmarks") if isinstance(analysis.get("landmarks"), dict) else {}
        ...
        for target, source in central.items():
            ...
            if value is not None:
                ...
                confidence[confidence_key] = max(
                    ...
                    float((records.get(source) or {}).get("rawConfidence") or 0.0),
                )
...
def run(input_path: Path, output_path: Path, metadata_path: Path):
    analysis_path, analysis = _load_analysis(input_path)
    ...
    profile.update({
        ...
        "analyzerStatus": analysis.get("status"),
        ...
        "skeletonPlanner": {
            "method": "autorig-v16-plus-approved-analyzer-v3.2-seeds",
            "fallback": "v16-geometry-only-for-unavailable-landmarks",
            "inventedLandmarks": 0,
        },
    })
    ...
    print(f"[clouva-autorig-analyzer-v3.2] {json.dumps(profile, separators=(',', ':'))}", flush=True)
    return profile
...
def main():
    ...
    if len(args) < 3:
        raise RuntimeError("Usage: autorig_avatar_v18.py input.glb output.glb metadata.json")
