...
def scale_avatar_to_centimeters(objects, target_height_cm: float):
    ...
    # Put the soles on Z=0 without changing the skeleton/mesh relationship.
...
def main():
    ...
    if len(values) < 2:
        raise RuntimeError("Usage: export_unreal.py input.glb output.fbx [target_height_cm]")
    ...
    print(
        f"[clouva-unreal] exported={output} original_height={original_height:.4f} "
        f"target_height_cm={target_height_cm:.2f} scale_factor={scale_factor:.4f}",
        flush=True,
    )
