...
export default function GarmentFlowClient() {
  ...
  const poll = async (taskId: string) => {
    while (true) {
      ...
      const response = await fetch(`/api/meshy/status?taskId=${encodeURIComponent(taskId)}`);
  ...
  const buttonText = phase === "measuring"
    ? "Analizando el GLB del avatar…"
    : phase === "previewing"
      ? `Meshy creando la forma · ${progress}%`
      : phase === "refining"
        ? `Meshy refinando y texturizando · ${progress}%`
        : phase === "saving"
          ? "Guardando la pieza…"
          : "Crear objeto 3D";
  ...
  return <main className="mx-auto min-h-screen w-full max-w-2xl px-4 pb-24 pt-5 text-white">
    <div className="mb-4 flex items-center justify-between">
      ...
      <span className="rounded-full border border-violet-400/25 bg-violet-400/10 px-3 py-1 text-[10px] tracking-[0.18em] text-violet-200">MESHY 3D</span>
    </div>

    {!result ? <section className="rounded-[28px] border border-white/10 bg-white/[0.035] p-4 sm:p-6">
      <div className="mb-5">
        <p className="mb-1 text-xs font-medium uppercase tracking-[0.22em] text-violet-300">Nueva pieza</p>
        <h1 className="text-2xl font-semibold leading-tight">Crear sobre el molde del avatar</h1>
        <p className="mt-2 text-sm leading-relaxed text-white/45">Sin OpenAI. La forma sale de la categoría, el fit, tu descripción y las medidas del GLB. El color y los detalles se aplican como textura.</p>
      </div>

      <div className="mb-5 grid grid-cols-3 gap-2">
        {CATEGORIES.map(([value, label]) => <button key={value} disabled={busy} onClick={() => setCategory(value)} className={`min-h-12 rounded-2xl border px-2 py-2 text-xs ${category === value ? "border-violet-400 bg-violet-400/15 text-white" : "border-white/10 bg-black/15 text-white/55"}`}>{label}</button>)}
      </div>
      ...
      <div className="space-y-3 rounded-3xl border border-white/10 bg-black/20 p-3">
        ...
        <div className="flex items-center justify-between rounded-2xl border border-white/10 bg-black/30 p-3">
          <div><p className="text-sm">Color base</p><p className="text-[11px] text-white/35">Se usa en la textura final</p></div>
          <input aria-label="Color base" type="color" value={color} disabled={busy} onChange={(e) => setColor(e.target.value)} className="h-11 w-14 bg-transparent" />
        </div>
        <div className="grid grid-cols-4 gap-2">
          {FITS.map((item) => <button key={item} disabled={busy} onClick={() => setFit(item)} className={`rounded-xl border px-2 py-2 text-[11px] ${fit === item ? "border-violet-400 bg-violet-400/15" : "border-white/10 text-white/50"}`}>{item}</button>)}
        </div>
        <label className="block">
          <span className="mb-1.5 block text-xs text-white/55">Forma y construcción</span>
          <textarea value={description} disabled={busy} onChange={(e) => setDescription(e.target.value)} rows={5} maxLength={600} placeholder="Ej: hoodie oversized, capucha profunda, mangas anchas, bolsillo canguro, puños ajustados…" className="w-full resize-none rounded-2xl border border-white/10 bg-black/30 p-4 text-sm outline-none" />
        </label>
        <label className="block">
          <span className="mb-1.5 block text-xs text-white/55">Textura y detalles opcionales</span>
          <textarea value={textureDetails} disabled={busy} onChange={(e) => setTextureDetails(e.target.value)} rows={4} maxLength={400} placeholder="Ej: algodón pesado mate, costuras violetas, logo pequeño bordado adelante, estampado grande atrás…" className="w-full resize-none rounded-2xl border border-violet-400/20 bg-violet-400/[0.04] p-4 text-sm outline-none" />
          <p className="mt-1 text-[10px] text-white/30">Esto modifica la superficie, no la forma del objeto.</p>
        </label>
      </div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <button onClick={() => { setResult(null); setPhase("idle"); }} className="rounded-2xl border border-white/15 py-3 text-sm">Crear otra</button>
        <Link href="/mi-flow/armario" className="rounded-2xl bg-violet-400 py-3 text-center text-sm font-semibold text-black">Ver en armario</Link>
      </div>
    </section>}
  </main>;
}
