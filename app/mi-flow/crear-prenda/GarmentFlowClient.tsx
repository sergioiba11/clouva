...
type Phase = "idle" | "measuring" | "previewing" | "texturing" | "saving" | "done" | "error";
...
export default function GarmentFlowClient() {
  ...
  const busy = ["measuring", "previewing", "texturing", "saving"].includes(phase);
  ...
  const generate3D = async () => {
    ...
    try {
      ...
      setPhase("texturing");
      setProgress(0);
      const texturePrompt = textureDetails.trim()
        ? `Main color ${color}. ${textureDetails}. Apply only as fabric, material, print, embroidery, patch or surface detail. Do not change the garment geometry.`
        : `Clean premium fabric in ${color}, subtle realistic textile detail, no logos or extra graphics.`;
      const refineResponse = await fetch("/api/meshy/refine", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ previewTaskId: created.taskId, texturePrompt }),
      });
      const refinedTask = await refineResponse.json();
      if (!refineResponse.ok || refinedTask.error) throw new Error(refinedTask.error || "No se pudo aplicar la textura.");

      const refined = await poll(refinedTask.taskId);
      if (refined.status !== "SUCCEEDED" || !refined.model_urls?.glb) throw new Error(refined.task_error?.message || "Meshy no devolvió el GLB texturizado.");
  ...
  const buttonText = phase === "measuring" ? "Analizando el GLB del avatar…" : phase === "previewing" ? `Meshy creando la forma · ${progress}%` : phase === "texturing" ? `Meshy aplicando textura · ${progress}%` : phase === "saving" ? "Guardando la pieza…" : "Crear objeto 3D";
  ...
  return <main className="mx-auto min-h-screen w-full max-w-2xl px-4 pb-24 pt-5 text-white">
    ...
    {!result ? <section className="rounded-[28px] border border-white/10 bg-white/[0.035] p-4 sm:p-6">
      ...
      <div className="space-y-3 rounded-3xl border border-white/10 bg-black/20 p-3">
        ...
        <div className="flex items-center justify-between rounded-2xl border border-white/10 bg-black/30 p-3"><div><p className="text-sm">Color base</p><p className="text-[11px] text-white/35">Se aplica en la etapa de textura</p></div><input aria-label="Color base" type="color" value={color} disabled={busy} onChange={(e) => setColor(e.target.value)} className="h-11 w-14 bg-transparent" /></div>
        ...
        <label className="block"><span className="mb-1.5 block text-xs text-white/55">Forma y construcción</span><textarea value={description} disabled={busy} onChange={(e) => setDescription(e.target.value)} rows={5} maxLength={600} placeholder="Ej: hoodie oversized, capucha profunda, mangas anchas, bolsillo canguro, puños ajustados…" className="w-full resize-none rounded-2xl border border-white/10 bg-black/30 p-4 text-sm outline-none" /></label>
        <label className="block"><span className="mb-1.5 block text-xs text-white/55">Textura y detalles opcionales</span><textarea value={textureDetails} disabled={busy} onChange={(e) => setTextureDetails(e.target.value)} rows={4} maxLength={400} placeholder="Ej: algodón pesado mate, costuras violetas, logo pequeño bordado en el pecho, diseño grande estampado atrás…" className="w-full resize-none rounded-2xl border border-violet-400/20 bg-violet-400/[0.04] p-4 text-sm outline-none" /><p className="mt-1 text-[10px] text-white/30">Esto no cambia la forma: solo material, color, estampado, bordado o parches.</p></label>
      </div>
