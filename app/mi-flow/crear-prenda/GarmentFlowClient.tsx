...
export default function GarmentFlowClient() {
  ...
  const poll = async (taskId: string) => {
    while (true) {
      ...
      if (typeof data.progress === "number") setProgress(Math.max(0, Math.min(100, Math.round(data.progress))));
  ...
  const phaseLabel =
    ...
    phase === "rigging" ? "Adaptando y riggeando al avatar…" :
    "Generar prenda 3D";
      ...
      {!result ? (
        <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-5">
          ...
          <div className="mb-4 grid grid-cols-3 gap-2 sm:grid-cols-4">
            {CATEGORIES.map(([value, label]) => (
              <button
                ...
                className={`rounded-xl border px-2 py-2 text-xs ${category === value ? "border-violet-400 bg-violet-400/15" : "border-white/10 text-white/60"}`}
              >
                {label}
              </button>
            ))}
          ...
          <div className="mb-3 flex flex-wrap gap-2">
            {FITS.map((item) => (
              <button
                ...
                type="button"
                onClick={() => setFit(item)}
                className={`rounded-full border px-3 py-1.5 text-xs ${fit === item ? "border-violet-400 bg-violet-400/15" : "border-white/10 text-white/60"}`}
              >
                {item}
              </button>
            ))}
          ...
          <textarea
            ...
            rows={5}
            placeholder="Ej: buzo negro oversized, mangas anchas, capucha profunda, costuras violetas y estética futurista…"
            className="mb-4 w-full rounded-xl border border-white/10 bg-black/25 p-3 text-sm"
          />
          ...
          <div className="mb-4 rounded-2xl border border-dashed border-white/15 bg-black/20 p-4">
            ...
            <p className="mb-3 mt-1 text-xs text-white/45">
              Subilo únicamente como arte para la textura. No define la forma de la prenda.
            </p>
            <label className="block cursor-pointer rounded-xl border border-white/10 bg-white/[0.03] p-3 text-center text-sm text-white/60">
              {artPreview ? (
                <img src={artPreview} alt="Arte para la prenda" className="mx-auto max-h-48 rounded-lg object-contain" />
              ) : (
                "Subir logo, portada o diseño"
              )}
              <input
                className="hidden"
                type="file"
            ...
            {art ? (
              <button
                type="button"
                onClick={() => { setArt(null); setArtPreview(null); }}
                className="mt-2 text-xs text-white/45 underline"
              >
                Quitar arte
              </button>
            ) : null}
          ...
          <button
            ...
            onClick={generate3D}
            disabled={busy}
            className="w-full rounded-2xl bg-violet-400 py-3 font-semibold text-black disabled:opacity-50"
          >
            {phaseLabel}
          </button>
          ...
          {busy ? (
            <div className="mt-3 h-2 overflow-hidden rounded-full bg-white/10">
              <div className="h-full rounded-full bg-violet-400 transition-all" style={{ width: `${Math.max(6, progress)}%` }} />
            </div>
          ) : null}
        ...
        <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-5">
          ...
          <div className="h-[430px] overflow-hidden rounded-2xl border border-white/10 bg-black/40">
            <OutfitPreview
              ...
              layers={[{ id: result.id, url: result.modelUrl, visible: true, category }]}
            />
          </div>
          <Link href="/mi-flow/armario" className="mt-3 block w-full rounded-2xl bg-violet-400 py-3 text-center text-sm font-semibold text-black">
            Ver en mis piezas
          </Link>
          <button
            type="button"
            onClick={() => { setResult(null); setPhase("idle"); setProgress(0); }}
            className="mt-2 w-full rounded-2xl border border-white/15 py-3 text-sm"
          >
            Crear otra pieza
          </button>
        </section>
