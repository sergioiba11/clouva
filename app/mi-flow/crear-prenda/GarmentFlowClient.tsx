...
export default function GarmentFlowClient() {
  ...
  const generateDesign = async () => {
    ...
    try {
      ...
      if (!description.trim()) throw new Error("Describí cómo querés la pieza y dónde va el detalle.");
      ...
      form.append("measurements", JSON.stringify(measured));
  ...
  const generate3D = async () => {
    ...
    try {
      if (!session?.access_token || !files.front || !files.back || !files.side) throw new Error("Primero generá las tres vistas con IA.");
      setPhase("uploading");
      const form = new FormData();
  ...
  const viewCard = (side: Side, title: string, subtitle: string) => (
    <div className="overflow-hidden rounded-2xl border border-white/10 bg-black/30 p-2">
      <div className="mb-2 flex items-center justify-between px-1"><span className="text-xs font-medium">{title}</span><span className="text-[10px] text-white/35">{subtitle}</span></div>
      {previews[side] ? <img src={previews[side]!} alt={title} className="aspect-square w-full rounded-xl bg-white/5 object-contain" /> : <div className="grid aspect-square place-items-center rounded-xl border border-dashed border-white/10 text-center text-xs text-white/35">La genera OpenAI<br/>automáticamente</div>}
    </div>
  );

  const designButtonText = phase === "measuring" ? "Analizando el GLB…" : phase === "designing" ? "OpenAI está creando las tres vistas…" : hasReferences ? "Regenerar las tres vistas" : "Generar diseño con IA";
  const meshButtonText = phase === "uploading" ? "Preparando referencias…" : phase === "generating" ? `Meshy está creando el 3D · ${progress}%` : phase === "saving" ? "Guardando pieza y portada…" : "Aprobar vistas y crear objeto 3D";
  ...
  return <main className="mx-auto min-h-screen w-full max-w-2xl px-4 pb-24 pt-5 text-white">
    ...
    {!result ? <section className="rounded-[28px] border border-white/10 bg-white/[0.035] p-4 sm:p-6">
      <div className="mb-5"><p className="mb-1 text-xs font-medium uppercase tracking-[0.22em] text-violet-300">Nueva pieza</p><h1 className="text-2xl font-semibold leading-tight">Diseñá una pieza para tu avatar</h1><p className="mt-2 text-sm leading-relaxed text-white/45">Elegís la forma y podés subir una imagen de logo, estampa o detalle. OpenAI genera frente, espalda y costado.</p></div>

      <div className="mb-5 grid grid-cols-3 gap-2">{CATEGORIES.map(([value, label]) => <button key={value} disabled={busy} onClick={() => { invalidateDesign(); setCategory(value); }} className={`min-h-12 rounded-2xl border px-2 py-2 text-xs ${category === value ? "border-violet-400 bg-violet-400/15 text-white" : "border-white/10 bg-black/15 text-white/55"}`}>{label}</button>)}</div>

      <div className="space-y-3 rounded-3xl border border-white/10 bg-black/20 p-3">
        <input value={name} disabled={busy} onChange={(e) => { invalidateDesign(); setName(e.target.value); }} placeholder="Nombre de la pieza" className="w-full rounded-2xl border border-white/10 bg-black/30 p-4 text-sm outline-none" />
        <div className="flex items-center justify-between rounded-2xl border border-white/10 bg-black/30 p-3"><div><p className="text-sm">Color base</p><p className="text-[11px] text-white/35">Material principal</p></div><input aria-label="Color base" type="color" value={color} disabled={busy} onChange={(e) => { invalidateDesign(); setColor(e.target.value); }} className="h-11 w-14 bg-transparent" /></div>
        <div className="grid grid-cols-4 gap-2">{FITS.map((item) => <button key={item} disabled={busy} onClick={() => { invalidateDesign(); setFit(item); }} className={`rounded-xl border px-2 py-2 text-[11px] ${fit === item ? "border-violet-400 bg-violet-400/15" : "border-white/10 text-white/50"}`}>{item}</button>)}</div>
        <textarea value={description} disabled={busy} onChange={(e) => { invalidateDesign(); setDescription(e.target.value); }} rows={5} maxLength={600} placeholder="Ej: hoodie oversized negro, logo pequeño adelante, diseño grande atrás, mangas anchas…" className="w-full resize-none rounded-2xl border border-white/10 bg-black/30 p-4 text-sm outline-none" />

        <label className="block cursor-pointer rounded-2xl border border-dashed border-violet-400/35 bg-violet-400/[0.06] p-3">
          <div className="flex items-center gap-3">
            {artworkPreview ? <img src={artworkPreview} alt="Detalle visual" className="h-20 w-20 rounded-xl bg-black/30 object-contain" /> : <div className="grid h-20 w-20 place-items-center rounded-xl bg-black/30 text-2xl">＋</div>}
            <div><p className="text-sm font-medium">Imagen del detalle</p><p className="mt-1 text-xs leading-relaxed text-white/40">Subí un logo, estampa, textura o dibujo. OpenAI lo aplicará a la pieza; no lo convertirá en el objeto completo.</p></div>
          </div>
      ...
      <button onClick={generateDesign} disabled={busy} className="my-4 w-full rounded-2xl bg-violet-400 py-4 font-semibold text-black disabled:opacity-50">{designButtonText}</button>
      {measurements ? <div className="mb-4 rounded-2xl border border-white/10 bg-white/[0.035] px-4 py-3 text-xs text-white/50">Molde detectado: {measurements.slotWidth.toFixed(2)} × {measurements.slotHeight.toFixed(2)} × {measurements.slotDepth.toFixed(2)} m</div> : null}

      <div className="mb-3"><p className="text-sm font-medium">Vistas generadas por IA</p><p className="mt-1 text-[11px] text-white/35">No se suben manualmente. OpenAI crea las tres y la frontal queda como portada.</p></div>
      <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-3">{viewCard("front", "Frente", "PORTADA")}{viewCard("back", "Espalda", "VISTA 2")}{viewCard("side", "Costado", "VISTA 3")}</div>

      {hasReferences ? <button onClick={generate3D} disabled={busy} className="w-full rounded-2xl bg-white py-4 font-semibold text-black disabled:opacity-30">{meshButtonText}</button> : null}
      {phase === "generating" ? <div className="mt-3 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full bg-violet-400" style={{ width: `${Math.max(2, progress)}%` }} /></div> : null}
