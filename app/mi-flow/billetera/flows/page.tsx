...
type FundingRef = { id: string; entry_type: string; provider: string | null; payment_method: string | null; amount: MoneyValue; currency: string | null; status: string; external_payment_id: string | null; provider_fee: MoneyValue; net_amount: MoneyValue; occurred_at: string; reserve_account_id?: string | null; custody_status?: string | null; custody_stage?: string | null; custody_reference?: string | null; custody_confirmed_at?: string | null; reference_usd_amount?: MoneyValue };
...
type ReserveAccountRef = { id: string; name: string; provider: string; accountType: string; accountRole?: string; currency: string; status: string; isActive: boolean; authorizedForFlow: boolean } | null;
...
function operationStage(operation: OperationRef) {
  ...
  const reserveDeposit = funding.find((entry) => entry.entry_type === "reserve_deposit" && entry.status === "confirmed");
  const processorFunding = funding.find((entry) => entry.entry_type === "funding" && entry.status === "confirmed");
  if (operation.status === "confirmed" && operation.issued_at && operation.backing_status === "verified") return "FLOW DISPONIBLE";
  if (reserveDeposit?.custody_stage === "allocated") return "FLOW DISPONIBLE";
  if (reserveDeposit?.custody_stage === "reserve_confirmed" || reserveDeposit?.custody_status === "confirmed") return "RESPALDO CONFIRMADO";
  if (processorFunding?.custody_stage === "transfer_pending") return "MOVIENDO A RESERVA";
  if (processorFunding?.custody_stage === "received_by_processor" || (operation.status === "confirmed" && operation.provider === "mercadopago")) return "PAGO RECIBIDO · esperando Reserva CLOUVA";
  if (operation.status === "confirmed" && operation.backing_status === "legacy_unverified") return "PENDING_BACKING · falta custodia real";
  if (operation.status === "confirmed") return "PAGO RECIBIDO · respaldo pendiente";
  if (operation.status === "pending" && operation.provider_payment_id) return "PAGO PENDIENTE · procesándose";
...
function activeBacking(asset: FlowAsset) {
  ...
  return Boolean(allocation && allocation.status === "active" && allocation.reserveAccount?.accountRole === "reserve" && allocation.reserveAccount.isActive && allocation.reserveAccount.authorizedForFlow && allocation.reserveAccount.status === "active");
}
...
function assetState(asset: FlowAsset) {
  ...
  if (activeBacking(asset) && asset.operation?.backing_status === "verified" && asset.operation.status === "confirmed") return "FLOW DISPONIBLE";
...
export default function FlowWalletAssetsPage() {
  ...
  useEffect(() => {
    ...
    if (returned === "success") setReturnMessage("Mercado Pago procesó el checkout. El FLOW seguirá bloqueado hasta que el dinero llegue y se confirme en la Reserva CLOUVA.");
    else if (returned === "pending") setReturnMessage("El pago está pendiente en Mercado Pago. Todavía no existe FLOW disponible.");
    else if (returned === "failure") setReturnMessage("Mercado Pago no confirmó el pago. No se emitió ningún FLOW.");
    if (!operation) return;
    const timers = [1500, 3500, 7000].map((delay) => window.setTimeout(() => void load(), delay));
  ...
  const processingFeeUsd = pricing.processingFeePolicy === "customer_buffer" ? roundMoney((requiredBackingUsd * pricing.processingFeeBps) / 10_000 + pricing.processingFeeFixedUsd) : 0;
  ...
  return <main className="min-h-screen bg-[#07060d] text-white"><MainNav/><div className="mx-auto max-w-5xl space-y-5 px-4 py-8 md:px-8">
    ...
    <section className="grid gap-4 md:grid-cols-2">{assets.map((asset) => {
      ...
      const processorFunding = fundingRows.find((entry) => entry.entry_type === "funding" && entry.status === "confirmed");
      const reserveDeposit = fundingRows.find((entry) => entry.entry_type === "reserve_deposit" && entry.status === "confirmed");
      ...
      return <article key={asset.id} className="rounded-[24px] border border-cyan-300/10 bg-[#0c0f14] p-5"><button onClick={() => setOpen(expanded ? null : asset.id)} className="w-full text-left"><div className="flex justify-between gap-3"><div><p className="text-[10px] uppercase tracking-[0.18em] text-white/30">Activo CLOUVA</p><h2 className="mt-1 text-2xl font-semibold">FLOW #{String(asset.flow_number ?? "—").padStart(6, "0")}</h2><p className="mt-1 text-sm text-white/45">Valor: US$ {pricing.flowUsdValue.toFixed(2)}</p></div><div className="flex items-center gap-2"><span className={`rounded-full border px-3 py-1 text-xs ${!isBacked ? "border-amber-200/20 text-amber-100" : "border-cyan-200/15 text-cyan-100"}`}>{assetState(asset)}</span>{expanded ? <ChevronUp size={16}/> : <ChevronDown size={16}/>}</div></div><div className="mt-4 grid gap-2 text-sm"><Row label="Propietario" value={player(asset.owner)}/><Row label="Origen" value={origin(operation)}/><Row label="Respaldo" value={isBacked ? "Reserva confirmada 1:1" : "No disponible para gastar"}/>{reserveAccount ? <Row label="Custodia" value={reserveAccount.name}/> : null}</div></button>
        ...
        {expanded ? <div className="mt-5 space-y-5 border-t border-white/[0.07] pt-5"><div><p className="text-[10px] uppercase tracking-[0.16em] text-white/30">Respaldo / Reserva</p><div className="mt-3 grid gap-2 text-sm"><Row label="Estado" value={assetState(asset)}/><Row label="Backing" value={allocation?.status === "active" ? "Confirmado" : "Pendiente"}/><Row label="Valor respaldado" value={allocation ? `US$ ${Number(allocation.referenceUsdValue).toFixed(2)}` : "—"}/><Row label="Custodia" value={reserveAccount?.name || "Sin Reserva asignada"}/><Row label="Proveedor de Reserva" value={reserveAccount?.provider || "—"}/><Row label="Asignado" value={when(allocation?.allocatedAt)}/></div></div><div><p className="text-[10px] uppercase tracking-[0.16em] text-white/30">Operación económica</p><div className="mt-3 grid gap-2 text-sm"><Row label="Método" value={method(operation)}/><Row label="Estado" value={paymentLabel}/><Row label="Backing requerido" value={operation?.required_backing_usd != null ? `US$ ${Number(operation.required_backing_usd).toFixed(2)}` : `US$ ${pricing.flowUsdValue.toFixed(2)}`}/><Row label="Monto checkout backing" value={operation?.backing_amount != null ? money(operation.backing_amount, operation.currency) : operation ? money(operation.amount, operation.currency) : "—"}/><Row label="Procesamiento" value={operation?.processing_fee_amount != null ? money(operation.processing_fee_amount, operation.currency) : "—"}/><Row label="Total cobrado" value={operation ? money(operation.amount, operation.currency) : "—"}/><Row label="Pago confirmado" value={when(operation?.confirmed_at)}/><Row label="FLOW respaldado" value={when(asset.backed_at)}/><Row label="Operación" value={operation?.id || "—"} mono/>{operation?.provider === "mercadopago" ? <Row label="Payment ID" value={operation.provider_payment_id || "—"} mono/> : null}{processorFunding ? <><Row label="Rail de cobro" value={`${money(processorFunding.amount, processorFunding.currency)} · ${processorFunding.custody_stage || "received_by_processor"}`}/><Row label="Fee proveedor real" value={money(processorFunding.provider_fee || 0, processorFunding.currency)}/><Row label="Neto registrado" value={money(processorFunding.net_amount ?? processorFunding.amount, processorFunding.currency)}/></> : null}{reserveDeposit ? <><Row label="Ingreso Reserva" value={`${money(reserveDeposit.amount, reserveDeposit.currency)} · ${reserveDeposit.custody_stage || reserveDeposit.custody_status || "—"}`}/><Row label="USD custodiado" value={reserveDeposit.reference_usd_amount != null ? `US$ ${Number(reserveDeposit.reference_usd_amount).toFixed(2)}` : "—"}/></> : null}{operation?.fx_rate_original_per_usd ? <Row label="Cotización histórica" value={`${operation.fx_pair || "USD/ARS"} · ${String(operation.fx_rate_original_per_usd)}`}/> : null}</div></div><div><p className="text-[10px] uppercase tracking-[0.16em] text-white/30">Origen del activo</p><div className="mt-3 grid gap-2 text-sm"><Row label="Comprador original" value={player(asset.originalBuyer)}/><Row label="FLOW emitido" value={when(asset.issued_at)}/><Row label="Operación origen" value={asset.originOperation?.id || "—"} mono/></div></div><div><p className="text-[10px] uppercase tracking-[0.16em] text-white/30">Comprobantes</p><div className="mt-3 grid gap-2 text-sm"><Row label="Registro interno" value={receipt?.document_number || "—"} mono/><Row label="Comprobante fiscal oficial" value={fiscal?.document_number || "No asociado todavía"}/></div></div><div><p className="text-[10px] uppercase tracking-[0.16em] text-white/30">Historial</p><div className="mt-2 space-y-2">{historyRows.map((history) => <div key={history.id} className="flex justify-between rounded-xl bg-white/[0.035] px-3 py-2 text-xs"><span>{String(history.action || "Movimiento")}</span><span className="text-white/35">{when(history.created_at)}</span></div>)}</div></div>{!isBacked ? <p className="rounded-xl border border-amber-300/15 bg-amber-300/[0.06] p-3 text-xs text-amber-100">Este FLOW no puede gastarse hasta que exista dinero real confirmado y asignado en una Reserva CLOUVA separada del procesador de pago.</p> : null}{operation?.refund_status ? <p className="rounded-xl border border-amber-300/15 bg-amber-300/[0.06] p-3 text-xs text-amber-100">El respaldo fue reembolsado o revertido; el caso queda auditado y el FLOW no se borra.</p> : null}</div> : null}
      </article>;
    })}</section>
  </div></main>;
}
