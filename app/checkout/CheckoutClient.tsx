...
export default function CheckoutClient() {
  ...
  return <main><MainNav/><section className="mx-auto w-full max-w-4xl px-4 py-12 md:px-8"><h1 className="text-3xl">Finalizar compra</h1><div className="mt-6 space-y-4 rounded-3xl border border-white/10 bg-white/[0.03] p-6 text-sm"><textarea value={address} onChange={e=>setAddress(e.target.value)} placeholder="Dirección" className="w-full rounded border border-white/20 bg-transparent p-2"/><select value={pay} onChange={e=>setPay(e.target.value)} className="w-full rounded border border-white/20 bg-transparent p-2">{methods.map(m=><option key={m.id} value={m.code}>{m.alias||m.code}</option>)}</select><p>Total: ${(total/100).toLocaleString("es-AR")}</p><button onClick={submit} className="rounded-full bg-white px-6 py-2 text-black">Confirmar pedido</button>{msg?<p>{msg}</p>:null}</div></section><MainFooter/></main>
}
