...
const EDITABLE_FIELDS = new Set([
  "name", "description", "price", "currency", "billingInterval",
  "benefits", "isActive", "isPublic", "displayOrder",
]);
    ...
    switch (key) {
      ...
      case "benefits":
        if (Array.isArray(value)) {
          patch.benefits = value.filter((item): item is string => typeof item === "string" && item.trim().length > 0).map((item) => item.trim().slice(0, 200)).slice(0, 20);
        }
        break;
      case "isActive":
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ slug: string; planId: string }> }) {
  try {
    ...
    if (plan.is_free && (patch.price !== undefined || patch.billing_interval !== undefined)) {
      throw new Error("Un plan gratuito no tiene precio ni frecuencia.");
    }
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ slug: string; planId: string }> }) {
  try {
    ...
    const { count, error: membersError } = await admin
      .from("studio_fan_memberships")
      .select("id", { count: "exact", head: true })
      .eq("plan_id", planId);
    if (membersError) throw new Error(membersError.message);
    if (count && count > 0) {
      const error = new Error("No se puede borrar un plan con socios activos. Desactivalo en su lugar.");
    ...
    const status = (error as Error & { status?: number })?.status ?? (isAuthError(error) ? 401 : 500);
    const message = error instanceof Error ? error.message : "No se pudo borrar el plan.";
