...
export async function POST(request: Request) {
  try {
    ...
    for (let step = 0; step < 8; step += 1) {
      ...
      const functionCall = parts.find((part: { functionCall?: unknown }) => part.functionCall)?.functionCall as
      ...
      if (functionCall.name === "get_repository_status") {
        ...
        toolResult = { ...file, content: file.content.slice(0, 50000) };
      ...
      if (pendingAction) {
        finalText = `${pendingAction.summary}\n\nRevisé el código necesario y preparé una propuesta para \`${pendingAction.path}\`. Tocá “Aplicar cambio” para crear el commit y disparar el deploy de Railway.`;
        break;
      }
    }
    ...
    return NextResponse.json({
      ok: true,
      message: finalText || "No pude generar una respuesta útil.",
      pendingAction,
      model,
    });
  } catch (error) {
