...
export async function POST(request: NextRequest) {
  try {
    ...
    if (recipient?.owner_user_id) {
      const { data: notification } = await admin
        ...
        .insert({
          ...
          title: `${inviter?.display_name || "Un Player"} te invitó a conectar agendas`,
          body: "CLOUVA Agenda",
          link: "/agenda/conexiones",
        })
        .select("id")
        .single();
      ...
      if (notificationId) {
        ...
        await saveDelivery(admin, body.agendaId, body.playerId, "notification", {
          status: "sent",
    ...
    const [email, whatsapp] = await Promise.all([
      sendAgendaInvitationEmail({ admin, agendaId: body.agendaId, playerId: body.playerId })
        .catch(() => ({ status: "failed" as const, reason: "EMAIL_DELIVERY_ERROR" })),
      sendAgendaInvitationWhatsApp({ admin, agendaId: body.agendaId, playerId: body.playerId })
        .catch(() => ({ status: "failed" as const, reason: "WHATSAPP_DELIVERY_ERROR" })),
    ]);
    ...
    await Promise.all([
      saveDelivery(admin, body.agendaId, body.playerId, "email", {
        ...email,
        providerMessageId: emailProviderMessageId,
      }),
      saveDelivery(admin, body.agendaId, body.playerId, "whatsapp", {
        ...whatsapp,
    ...
    return NextResponse.json(
      { ...result, emailDelivery: email, whatsappDelivery: whatsapp },
      { status: 201 },
    );
  } catch (error) {
