...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const customer = {
      name: cleanText(body.customer?.name, 160),
      phone: cleanText(body.customer?.phone, 60),
      email: cleanText(body.customer?.email, 320).toLowerCase(),
      address: cleanText(body.customer?.address, 500),
    };

    if (!customer.name || !customer.email || !customer.address) {
      return NextResponse.json({ error: "Completá nombre, email y dirección de entrega." }, { status: 400 });
    }
    ...
    for (const productId of productIds) {
      ...
      if (!product || !product.active || product.status !== "activo") {
    ...
    await admin.from("order_status_history").insert({
      ...
      note: "Checkout de Mercado Pago creado.",
    });
    ...
    if (createdOrderId) {
      await admin
        .from("orders")
        .update({ payment_status: "cancelado", shipping_status: "cancelado", status: "cancelado" })
