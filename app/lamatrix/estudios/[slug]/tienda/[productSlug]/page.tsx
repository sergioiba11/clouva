...
export default async function MatrixStudioProductPage({ params }: { params: Promise<{ slug: string; productSlug: string }> }) {
  const { slug, productSlug } = await params;
  const result = await resolveStudioAlias(slug);
  if (!result) notFound();
  if (slug.toLowerCase() !== result.canonicalAlias.toLowerCase()) permanentRedirect(`${result.publicStudio.href}/tienda/${encodeURIComponent(productSlug)}`);

  const supabase = createPublicSupabase();
  const { data: spot } = await supabase.from("commerce_spots").select("id,name").eq("studio_id", result.studio.id).eq("status", "active").eq("public_enabled", true).limit(1).maybeSingle();
  if (!spot) notFound();
  const { data } = await supabase.from("commerce_products").select(commerceProductSelect).eq("spot_id", spot.id).eq("slug", productSlug).eq("status", "published").maybeSingle();
  if (!data) notFound();
  ...
  if (product.product_type === "bundle") {
    const { data: components } = await supabase.from("commerce_listing_components").select("component_listing_id,quantity,component_role").eq("bundle_listing_id", product.id);
    const componentIds = (components ?? []).map((component) => component.component_listing_id);
    if (componentIds.length) {
      const { data: componentProducts } = await supabase.from("commerce_products").select("id,name,product_type").in("id", componentIds).eq("status", "published");
