...
export default async function MatrixPage() {
  ...
  return (
    <PublicShell brand="LA MATRIX" brandHref="/lamatrix" navLinks={matrixNav}>
      <section
      ...
      >
        ...
        <div className="mx-auto max-w-6xl">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h1 className="mt-7 text-5xl font-black tracking-[-0.06em] text-white sm:text-7xl lg:text-8xl">LA MATRIX</h1>
            <p className="mx-auto mt-5 max-w-2xl text-sm leading-7 text-white/60 sm:text-base">
              Encontrá Players y Estudios conectados por la música, la creación y los proyectos. Cada identidad mantiene su propia voz y se conecta con una red real.
            </p>
          </div>

          <div className="mx-auto mt-12 grid max-w-4xl gap-5 md:grid-cols-2">
            <Link href="/players" className="group relative overflow-hidden rounded-[2rem] border border-violet-400/20 bg-[linear-gradient(145deg,rgba(28,18,58,.9),rgba(8,7,18,.92))] p-7 shadow-[0_24px_80px_rgba(0,0,0,.35)] transition duration-300 hover:-translate-y-1 hover:border-violet-300/50">
              ...
              <div className="relative flex items-start justify-between gap-5">
                <span className="grid h-14 w-14 place-items-center rounded-2xl border border-violet-400/30 bg-violet-500/15 text-violet-200 shadow-[0_0_35px_rgba(124,58,237,.2)]"><UsersRound size={27} /></span>
                <span className="rounded-full border border-white/10 px-3 py-1 text-[10px] uppercase tracking-wider text-white/45">{players.length ? `${players.length} publicados` : "Directorio"}</span>
              </div>
              <h2 className="relative mt-7 text-3xl font-bold tracking-tight">PLAYERS</h2>
              <p className="relative mt-3 max-w-sm text-sm leading-6 text-white/55">Artistas, productores y creadores con una identidad pública propia.</p>
              <span className="relative mt-8 inline-flex items-center gap-2 rounded-full bg-violet-500 px-5 py-2.5 text-xs font-semibold text-white transition group-hover:bg-violet-400">Explorar Players <ArrowRight size={14} /></span>
            </Link>

            <Link href="/lamatrix/estudios" className="group relative overflow-hidden rounded-[2rem] border border-white/12 bg-[linear-gradient(145deg,rgba(19,16,38,.9),rgba(8,7,18,.92))] p-7 shadow-[0_24px_80px_rgba(0,0,0,.35)] transition duration-300 hover:-translate-y-1 hover:border-violet-300/45">
              ...
              <div className="relative flex items-start justify-between gap-5">
                <span className="grid h-14 w-14 place-items-center rounded-2xl border border-violet-400/25 bg-violet-500/10 text-violet-200"><Building2 size={27} /></span>
                <span className="rounded-full border border-white/10 px-3 py-1 text-[10px] uppercase tracking-wider text-white/45">{studios.length ? `${studios.length} publicados` : "Directorio"}</span>
              </div>
              <h2 className="relative mt-7 text-3xl font-bold tracking-tight">ESTUDIOS</h2>
              <p className="relative mt-3 max-w-sm text-sm leading-6 text-white/55">Estudios, sellos, colectivos y espacios creativos con identidad propia.</p>
              <span className="relative mt-8 inline-flex items-center gap-2 rounded-full border border-violet-300/30 bg-violet-500/10 px-5 py-2.5 text-xs font-semibold text-violet-100 transition group-hover:bg-violet-500/20">Explorar Estudios <ArrowRight size={14} /></span>
            </Link>
          </div>
            ...
            {[
              ...
              return (
                <article key={item.title} className="flex cursor-not-allowed items-center gap-3 rounded-2xl border border-white/[0.06] bg-zinc-900/55 p-4 text-white/30 grayscale">
                  ...
                  <div><h3 className="text-sm font-semibold">{item.title}</h3><p className="mt-0.5 text-[10px] uppercase tracking-wider">Próximamente</p></div>
                </article>
              );
            })}
          </section>
