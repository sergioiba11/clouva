  ...
  return (
    <PublicShell brand="LA MATRIX" brandHref="/lamatrix" navLinks={[
      ...
      <section className="relative overflow-hidden border-b border-white/10 px-4 py-16 sm:px-6" data-visual-asset="players-directory-hero-01">
        <div className="absolute inset-0 bg-cover bg-center opacity-70" style={{ backgroundImage: `url(${VISUAL_ASSETS["players-directory-hero-01"]})` }} aria-hidden="true" />
      ...
      <section className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
        {players.length === 0 ? (
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {players.map((player) => (
              <Link key={player.id} href={`/${player.slug}`} className="group overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.025] transition duration-300 hover:-translate-y-1 hover:border-violet-400/50">
                <div className="relative h-52 bg-white/[0.03]">
                  {player.cover_url || player.profile_image_url ? <img src={player.cover_url || player.profile_image_url || ""} alt={player.display_name} className="h-full w-full object-cover transition duration-500 group-hover:scale-[1.03]" /> : <div className="flex h-full items-center justify-center bg-gradient-to-br from-violet-500/25 to-black text-5xl font-semibold text-white/50">{player.display_name.charAt(0)}</div>}
                  <div className="absolute inset-0 bg-gradient-to-t from-[#07060b] via-transparent to-transparent" />
                </div>
                <div className="p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div><h2 className="text-xl font-semibold">{player.display_name}</h2>{player.username ? <p className="mt-1 text-xs text-white/40">@{player.username.replace(/^@/, "")}</p> : null}</div>
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">{[...(player.professional_categories || []), ...(player.primary_role ? [player.primary_role] : [])].slice(0, 3).map((label) => <span key={label} className="rounded-full border border-white/10 px-2.5 py-1 text-[11px] text-white/50">{label}</span>)}</div>
                </div>
              </Link>
            ))}
          </div>
