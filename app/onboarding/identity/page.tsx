...
const OPTIONS = [
  ["Player", "Música y obra propia"],
  ["Artista", "Identidad artística"],
  ["Productor", "Producción musical"],
  ["Creador", "Contenido y proyectos"],
  ["Manager", "Gestión artística"],
  ["Diseñador", "Imagen y dirección visual"],
  ["Estudio o colectivo", "Espacio creativo"],
  ["Solo explorar", "Entrar sin publicar"],
] as const;
...
export default function IdentityOnboardingPage() {
  ...
  const toggle = (value: string) => {
    setSelected((current) => current.includes(value) ? current.filter((item) => item !== value) : [...current.filter((item) => item !== "Solo explorar"), value]);
  };
  ...
  return (
    <OnboardingShell
      step={2}
      title="¿Cómo querés presentarte en CLOUVA?"
      description="Podés elegir más de una opción y modificarla cuando quieras."
    >
      ...
      <button disabled={saving || loading} onClick={() => void continueOnboarding()} className="mt-6 w-full rounded-xl bg-violet-600 px-5 py-3.5 font-semibold transition hover:bg-violet-500 disabled:opacity-60">{saving ? "Creando tu perfil..." : "Crear mi perfil"}</button>
    </OnboardingShell>
  );
}
