...
export default function EstudiosMembresiasAdminPage() {
  const [tab, setTab] = useState<"miembros" | "planes">("miembros");
  ...
  const load = async () => {
    ...
    const [membersResult, plansResult] = await Promise.all([
      supabase
        ...
        .select("id,user_id,status,public_role_label,area_label,joined_at,studios(name,slug),studio_membership_plans(name,is_free,price,currency)")
        .order("joined_at", { ascending: false })
        .limit(500),
      supabase
        .from("studio_membership_plans")
        .select("id,studio_id,name,public_role_label,area_label,is_free,price,currency,billing_interval,is_active,studios(name,slug)")
    ...
    const memberRows = (membersResult.data ?? []) as unknown as Omit<MembershipRow, "profiles">[];
    ...
    setPlans(planRows);
    setDrafts(Object.fromEntries(planRows.filter((plan) => !plan.is_free).map((plan) => [plan.id, { price: String(plan.price ?? ""), currency: plan.currency, billingInterval: plan.billing_interval ?? "month" }])));
    setLoading(false);
  };
  ...
  const stats = useMemo(() => {
    ...
    const paying = active.filter((member) => member.studio_membership_plans && !member.studio_membership_plans.is_free);
  ...
  const savePlan = async (plan: PlanRow) => {
    ...
    setError(null);
    const { supabase } = await import("@/lib/supabase");
    const { error: updateError } = await supabase.from("studio_membership_plans").update({ price, currency: draft.currency.trim().toUpperCase() || "ARS", billing_interval: draft.billingInterval }).eq("id", plan.id);
    setSavingId(null);
  ...
  return (
    <div className="space-y-4">
      <PremiumCard className="p-6">
        ...
        <p className="mt-1 text-sm text-white/50">Roles públicos, áreas, planes gratis o pagos y estado real de cada miembro. Los permisos administrativos siguen separados.</p>
      </PremiumCard>

      <div className="grid gap-3 sm:grid-cols-3">
        <StatCard label="Membresías totales" value={loading ? "…" : stats.total} />
        <StatCard label="Activas" value={loading ? "…" : stats.active} />
        <StatCard label="Planes pagos activos" value={loading ? "…" : stats.paying} />
      </div>

      <div className="flex gap-2">
        <button onClick={() => setTab("miembros")} className={`rounded-xl px-4 py-2 text-sm ${tab === "miembros" ? "bg-violet-600 text-white" : "border border-white/10 text-white/60"}`}>Miembros</button>
      ...
      {tab === "miembros" ? (
        <PremiumCard className="p-5">
          {loading ? <p className="text-sm text-white/50">Cargando miembros…</p> : null}
          {!loading && members.length === 0 ? <p className="text-sm text-white/50">Todavía no hay miembros en ningún Estudio.</p> : null}
          <div className="space-y-2">
            {members.map((member) => {
              const plan = member.studio_membership_plans;
              return (
                <div key={member.id} className="grid gap-2 rounded-xl border border-white/10 bg-black/20 p-3 text-sm md:grid-cols-7 md:items-center">
                  ...
                  <span className="text-violet-200">{member.public_role_label || "Miembro"}</span>
                  <span className="text-white/45">{member.area_label || "—"}</span>
        ...
        <PremiumCard className="p-5">
          ...
          <div className="space-y-2">
            {plans.map((plan) => {
              const draft = drafts[plan.id];
              return (
                <div key={plan.id} className="grid gap-2 rounded-xl border border-white/10 bg-black/20 p-3 text-sm md:grid-cols-8 md:items-center">
                  <span className="text-white/60">{plan.studios?.name ?? "—"}</span>
                  <span>{plan.name}</span>
                  <span className="text-violet-200">{plan.public_role_label}</span>
                  <span className="text-white/45">{plan.area_label}</span>
                  {plan.is_free ? (
