...
export default function Page(){
 ...
 const load=async()=>{const {supabase}=await import("@/lib/supabase");const {data}=await supabase.from("profiles").select("id,full_name,role,role_v2").in("role_v2",["admin","empleado","cliente","vip"]);setRows(data??[])};
 useEffect(()=>{void load();},[]);
 const makeEmployee=async()=>{const {supabase}=await import("@/lib/supabase");const {data:u}=await supabase.from("customers").select("profile_id,email").eq("email",email).maybeSingle();if(!u?.profile_id)return;await supabase.from("profiles").update({role_v2:"empleado",role:"customer"}).eq("id",u.profile_id);setEmail("");void load();};
