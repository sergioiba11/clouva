      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        <section className="rounded-2xl border border-white/15 p-4">
          ...
          <ul className="space-y-1 text-sm text-white/80">
            ...
            <li>role: <b>{role}</b></li>
            <li>admin access: <b>{String(adminAccess)}</b></li>
          </ul>
        </section>

        <section className="rounded-2xl border border-white/15 p-4">
          <h2 className="mb-2 font-medium">User</h2>
          <pre className="overflow-auto rounded-xl bg-black/30 p-3 text-xs text-white/80">{JSON.stringify(user, null, 2)}</pre>
        </section>

        <section className="rounded-2xl border border-white/15 p-4 md:col-span-2">
          <h2 className="mb-2 font-medium">Session</h2>
          <pre className="overflow-auto rounded-xl bg-black/30 p-3 text-xs text-white/80">{JSON.stringify(session, null, 2)}</pre>
        </section>

        <section className="rounded-2xl border border-white/15 p-4 md:col-span-2">
          <h2 className="mb-2 font-medium">Profile</h2>
          <pre className="overflow-auto rounded-xl bg-black/30 p-3 text-xs text-white/80">{JSON.stringify(profile, null, 2)}</pre>
        </section>
      </div>
    </main>
  );
