...
import { StudioIdentityWorkspace } from "@/components/studio/StudioIdentityWorkspace";
...
const SECTION_META: Record<Section, SectionMeta> = {
  ...
  "Identidad IA": { title: "Identidad", description: "Diseñá la página pública con referencias, Gemini, draft y preview real en un mismo workspace." },
...
export default function StudioDashboardPage({ params }: { params: Promise<{ studioId: string }> }) {
  ...
  useTrebolContextRegistration({
    ...
    data: {
      ...
      studioDesigner: section === "Identidad IA",
    },
  });
  ...
  const addService = async () => {
    if (!newService.name.trim()) { setError("El nombre del servicio es obligatorio."); return; }
    setWorkingId("new-service");
    setError(null);
    try {
      const response = await authenticatedFetch(`/api/studios/${encodeURIComponent(studioId)}/services`, {
        method: "POST",
        body: JSON.stringify({ name: newService.name, description: newService.description || null, category: newService.category || null, priceType: newService.priceType, price: newService.priceType === "fixed" ? Number(newService.price) : null, ctaType: newService.ctaType }),
      });
    ...
    } catch (addError) { setError(addError instanceof Error ? addError.message : "No se pudo agregar el servicio."); } finally { setWorkingId(null); }
  };

  const toggleService = async (service: ServiceRow) => {
    setWorkingId(service.id); setError(null);
    try {
      const response = await authenticatedFetch(`/api/studios/${encodeURIComponent(studioId)}/services/${service.id}`, { method: "PATCH", body: JSON.stringify({ isActive: !service.is_active }) });
      const payload = await readApiJson<{ service: ServiceRow }>(response);
      setServices((current) => current.map((item) => item.id === service.id ? payload.service : item));
    } catch (toggleError) { setError(toggleError instanceof Error ? toggleError.message : "No se pudo actualizar el servicio."); } finally { setWorkingId(null); }
  };

  const deleteService = async (serviceId: string) => {
    if (!window.confirm("¿Borrar este servicio?")) return;
    setWorkingId(serviceId); setError(null);
    try { await authenticatedFetch(`/api/studios/${encodeURIComponent(studioId)}/services/${serviceId}`, { method: "DELETE" }); setServices((current) => current.filter((item) => item.id !== serviceId)); }
    catch (deleteError) { setError(deleteError instanceof Error ? deleteError.message : "No se pudo borrar el servicio."); } finally { setWorkingId(null); }
  };

  const addPlan = async () => {
    if (!newPlan.name.trim()) { setError("El nombre del plan es obligatorio."); return; }
    setWorkingId("new-plan"); setError(null);
    try {
      const response = await authenticatedFetch(`/api/studios/${encodeURIComponent(studioId)}/membership-plans`, { method: "POST", body: JSON.stringify({ name: newPlan.name, description: newPlan.description || null, isFree: newPlan.isFree, price: newPlan.isFree ? null : Number(newPlan.price), billingInterval: newPlan.isFree ? null : newPlan.billingInterval, benefits: newPlan.benefits.split("\n").map((line) => line.trim()).filter(Boolean) }) });
    ...
    } catch (addError) { setError(addError instanceof Error ? addError.message : "No se pudo crear el plan."); } finally { setWorkingId(null); }
  };

  const togglePlan = async (plan: PlanRow) => {
    setWorkingId(plan.id); setError(null);
    try {
      const response = await authenticatedFetch(`/api/studios/${encodeURIComponent(studioId)}/membership-plans/${plan.id}`, { method: "PATCH", body: JSON.stringify({ isActive: !plan.is_active }) });
      const payload = await readApiJson<{ plan: PlanRow }>(response);
      setPlans((current) => current.map((item) => item.id === plan.id ? payload.plan : item));
    } catch (toggleError) { setError(toggleError instanceof Error ? toggleError.message : "No se pudo actualizar el plan."); } finally { setWorkingId(null); }
  };

  const deletePlan = async (planId: string) => {
    if (!window.confirm("¿Borrar este plan?")) return;
    setWorkingId(planId); setError(null);
    try { await authenticatedFetch(`/api/studios/${encodeURIComponent(studioId)}/membership-plans/${planId}`, { method: "DELETE" }); setPlans((current) => current.filter((item) => item.id !== planId)); }
    catch (deleteError) { setError(deleteError instanceof Error ? deleteError.message : "No se pudo borrar el plan."); } finally { setWorkingId(null); }
  };
  ...
  const review = async (applicationId: string, status: "in_review" | "accepted" | "rejected") => {
    ...
    } catch (reviewError) { setError(reviewError instanceof Error ? reviewError.message : "No se pudo revisar la solicitud."); } finally { setWorkingId(null); }
  };
  ...
  const openSection = (item: Section) => {
    if (item === "Comercio") { router.push(`/studio-dashboard/${studioId}/commerce`); return; }
    setSection(item);
  };
  ...
  return (
    <main className="min-h-screen bg-[#05040a] text-white" data-clouva-component="StudioDashboardWorkspace">
      <header className="sticky top-0 z-40 border-b border-white/10 bg-[#06050b]/95 shadow-[0_20px_70px_rgba(0,0,0,0.22)] backdrop-blur-2xl">
        <div className="mx-auto flex w-full max-w-[1760px] flex-col gap-3 px-4 py-3 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0"><div className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-violet-400" /><p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-violet-200/60">Studio Dashboard</p></div><h1 className="truncate text-base font-semibold sm:text-lg">{studioName}</h1></div>
          </div>
          <div className="flex items-center gap-2 overflow-x-auto pb-1 lg:overflow-visible lg:pb-0">
            ...
            {profileSaveVisible ? <StatusChip tone="amber">Perfil sin guardar</StatusChip> : null}
            ...
            {profileSaveVisible ? <button disabled={workingId === "profile"} onClick={() => void saveStudio()} className="shrink-0 rounded-xl border border-white/15 bg-white/[0.04] px-3.5 py-2 text-xs font-semibold text-white/80 transition hover:bg-white/[0.08] disabled:opacity-40">{workingId === "profile" ? "Guardando…" : "Guardar perfil"}</button> : null}
            <Link href={`/studio-dashboard/${studioId}/identity-preview`} target="_blank" className="shrink-0 rounded-xl border border-white/15 px-3.5 py-2 text-xs font-semibold text-white/80 transition hover:bg-white/[0.05]">Preview</Link>
      ...
      <div className="mx-auto grid w-full max-w-[1760px] gap-5 px-4 py-5 sm:px-6 lg:grid-cols-[250px_minmax(0,1fr)] lg:px-8">
        <aside className="min-w-0 lg:sticky lg:top-[86px] lg:h-[calc(100vh-106px)]">
          <div className="rounded-2xl border border-white/[0.08] bg-[#0a0810] p-2.5 lg:flex lg:h-full lg:flex-col lg:p-3">
            <div className="hidden px-2 pb-3 pt-1 lg:block"><p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-white/30">Navegación</p><p className="mt-1 text-xs leading-5 text-white/40">Todo el Studio desde un solo lugar.</p></div>
            <nav className="flex gap-3 overflow-x-auto lg:block lg:min-h-0 lg:flex-1 lg:overflow-y-auto lg:pr-1 lg:[scrollbar-width:none] lg:[&::-webkit-scrollbar]:hidden">
              {NAV_GROUPS.map((group) => <div key={group.label} className="flex shrink-0 gap-2 lg:mb-4 lg:block"><p className="mb-1.5 hidden px-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-white/25 lg:block">{group.label}</p><div className="flex gap-2 lg:flex-col">{group.items.map((item) => { const active = section === item; return <button key={item} onClick={() => openSection(item)} className={`group relative flex shrink-0 items-center justify-between gap-3 rounded-xl px-3.5 py-2.5 text-left text-sm transition lg:w-full ${active ? "border border-violet-400/25 bg-violet-500/15 text-white shadow-[inset_3px_0_0_#8b5cf6]" : "border border-transparent text-white/52 hover:border-white/[0.08] hover:bg-white/[0.035] hover:text-white"}`}><span>{item}</span>{item === "Solicitudes" && pendingApplications.length ? <span className="rounded-full bg-violet-500/25 px-2 py-0.5 text-[10px] font-semibold text-violet-100">{pendingApplications.length}</span> : null}</button>; })}</div></div>)}
            </nav>
          </div>
        </aside>

        <section className="min-w-0">
          {section !== "Identidad IA" ? <div className="mb-5 flex flex-col gap-3 rounded-2xl border border-white/[0.07] bg-white/[0.018] px-5 py-4 sm:flex-row sm:items-end sm:justify-between sm:px-6"><div className="min-w-0"><p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-violet-200/50">Workspace · {studioName}</p><h2 className="mt-1 text-2xl font-semibold tracking-[-0.02em] sm:text-3xl">{sectionMeta.title}</h2><p className="mt-1.5 max-w-3xl text-sm leading-6 text-white/45">{sectionMeta.description}</p></div><div className="flex shrink-0 items-center gap-2 text-[11px] text-white/35"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400" />Contexto del Studio activo</div></div> : null}

          {section === "Resumen" ? <div className="space-y-5"><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Solicitudes pendientes" value={pendingApplications.length} /><Metric label="Players" value={data.players.length} /><Metric label="Miembros internos" value={data.members.length} /><Metric label="Proyectos" value={data.projects.length} /></div><SpotQrCard entityName={studioName} publicPath={publicSpotPath} isPublic={spotIsPublic} compact /><Panel title="Actividad que requiere atención" eyebrow="Pendientes"><div className="space-y-3">{pendingApplications.slice(0, 5).map((application) => <ApplicationRow key={String(application.id)} application={application} working={workingId === application.id} onReview={review} />)}{pendingApplications.length === 0 ? <p className="text-sm text-white/45">No hay solicitudes pendientes.</p> : null}</div></Panel><Panel title="Miembros por rol" eyebrow="Equipo"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{Object.entries(memberCounts).map(([role, count]) => <div key={role} className="rounded-2xl border border-white/[0.08] bg-black/20 p-5"><p className="text-sm capitalize text-white/50">{role}</p><p className="mt-2 text-3xl font-semibold">{count}</p></div>)}</div></Panel></div> : null}
          ...
          {section === "Identidad IA" ? <StudioIdentityWorkspace studioId={studioId} studioSlug={studioSlug} studioName={studioName} /> : null}
