...
export function ResultRigPreview({ url, showAvatar = true, category, onInfo }: Props) {
  ...
  useEffect(() => {
    ...
    void Promise.all([loader.loadAsync(url), avatarPromise]).then(([rigGltf, avatarGltf]) => {
      ...
      if (!avatarModel) throw new Error("No hay un avatar activo disponible para vestir.");
      ...
      avatarBounds = visibleMeshBounds(avatarModel).clone();
      if (avatarBounds.isEmpty()) throw new Error("El avatar activo no contiene una malla visible.");
      fitCameraOnce(avatarBounds);

      const exportedTransform = transformDiagnostics(rigModel);
      if (!isIdentityRoot(rigModel)) throw new Error(RIG_ERROR);
      displayGroup.add(rigModel);
      displayGroup.updateMatrixWorld(true);

      const initialGarmentBounds = visibleMeshBounds(rigModel);
      const bounds = validateBounds(avatarBounds, initialGarmentBounds);
      avatarHeight = bounds.avatarHeight;
      const bindPose = compareBindPose(avatarModel, rigModel, avatarHeight);
      const shared = bindGarmentToAvatar(rigModel, avatarModel);
      const postBindBounds = animatedGarmentBounds(rigModel);
      validateBounds(avatarBounds, postBindBounds);

      const primaryBones = collectBones(avatarModel);
      const helper = new SkeletonHelper(avatarModel);
      ...
      proceduralMotionRef.current = createProceduralMotion(avatarModel);
      ...
      console.info("[CLOUVA rig diagnostics]", {
        avatarRoot: transformDiagnostics(avatarModel),
        garmentRoot: exportedTransform,
