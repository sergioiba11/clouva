...
type RigExecutionResult = {
  previewResultUrl: string;
  libraryResultUrl: string;
};
...
export function CreatorStudioPro() {
  ...
  function resetProject() {
    ...
    setProgress(0);
    setError(null);
    setStatus("Elegí un GLB para comenzar.");
    resetFit();
  }
  ...
  async function runDeformableRig(): Promise<RigExecutionResult> {
    ...
    const directUrl = typeof data.resultUrl === "string" ? data.resultUrl : null;
    if (directUrl) return { previewResultUrl: directUrl, libraryResultUrl: directUrl };
    ...
    return { previewResultUrl: finalStatus.resultUrl, libraryResultUrl: finalStatus.resultUrl };
  }

  async function runRigidRig(): Promise<RigExecutionResult> {
    ...
    const riggedObject = await pollJob(String(rigData.jobId));
    if (!riggedObject.resultUrl) throw new Error("El rig del accesorio terminó sin generar un GLB.");
  ...
  async function applySmartRig() {
    ...
    try {
      const execution = profile.pipeline === "garment" ? await runDeformableRig() : await runRigidRig();
      setProgress(92);
      setStatus("Guardando el asset validado en tu biblioteca…");
      const readyAsset = await saveRiggedReferenceAsset(asset.id, execution.libraryResultUrl, previewSettings);
      const readyPreviewUrl = URL.createObjectURL(readyAsset.file);
      if (previewUrl?.startsWith("blob:")) URL.revokeObjectURL(previewUrl);
      setPreviewUrl(readyPreviewUrl);
      setAsset(readyAsset);
      setResultUrl(execution.previewResultUrl);
      setProgress(100);
      setStatus("Rig terminado y guardado. El asset ya usa el perfil correcto del avatar.");
      setStep("result");
    } catch (cause) {
  ...
  return (
    <main className="min-h-screen bg-[#060408] pb-28 text-white">
      <header className="sticky top-0 z-40 border-b border-white/10 bg-[#08050c]/95 px-3 py-3 backdrop-blur-xl sm:px-6">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-violet-300"><WandSparkles className="h-4 w-4" /> Creator Studio</div>
            <p className="truncate text-sm text-white/55">Un modelo, un rig correcto, un resultado claro.</p>
          </div>
          {asset ? <div className="max-w-[42vw] truncate rounded-full border border-violet-400/25 bg-violet-500/10 px-3 py-2 text-xs font-bold text-violet-200">{asset.name}</div> : null}
        </div>
      </header>
      ...
      <div className="mx-auto max-w-6xl px-3 py-4 sm:px-6 sm:py-7">
        <nav className="mb-4 grid grid-cols-4 gap-2 rounded-2xl border border-white/10 bg-white/[0.035] p-2 sm:mb-6">
          {STEPS.map((item, index) => {
            ...
            return (
              <button key={item.id} type="button" disabled={index > 0 && !asset} onClick={() => { if (index === 0 || asset) setStep(item.id); }} className={`rounded-xl px-2 py-3 text-center transition ${active ? "bg-violet-600 text-white shadow-lg shadow-violet-950/40" : completed ? "bg-emerald-500/10 text-emerald-200" : "text-white/45 disabled:opacity-30"}`}>
                <span className="mx-auto mb-1 flex h-6 w-6 items-center justify-center rounded-full border border-current text-[11px] font-black">{completed ? "✓" : index + 1}</span>
        ...
        {step === "asset" ? (
          <section>
            <div className="mb-4 rounded-2xl border border-violet-400/20 bg-gradient-to-br from-violet-600/15 to-transparent p-4 sm:p-6">
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-white/60">CLOUVA detecta la categoría y activa el rig correcto. Las prendas copian pesos del cuerpo; los accesorios se anclan al hueso indicado.</p>
            </div>
            <ReferenceAssetLibrary selectedAssetId={asset?.id ?? null} onCategoryChange={setCategory} onSelect={(nextAsset, url) => {
              setAsset(nextAsset);
              setPreviewUrl(url);
              setResultUrl(null);
              setError(null);
              if (nextAsset) {
                ...
                setStatus(`${nextAsset.name} listo. Perfil ${resolveRigProfile(nextAsset.category).label} activado.`);
              }
            }} />
            {asset ? <PrimaryButton onClick={() => setStep("fit")}>Ajustar {asset.name} <ArrowRight className="h-5 w-5" /></PrimaryButton> : null}
          </section>
        ) : null}
          ...
          <section className="grid gap-4 lg:grid-cols-[minmax(0,1.3fr)_minmax(300px,.7fr)]">
            <div className="overflow-hidden rounded-3xl border border-white/10 bg-[#100918]">
              <div className="h-[58vh] min-h-[480px] max-h-[760px]">
                <SmartTryOnViewer category={category} fit={fit} pose={pose} view={view} background="#100918" showBody garmentOnly={false} adjustments={adjustments} referenceModelUrl={previewUrl} anchorBoneKey={profile.mode === "rigid" ? anchorBoneKey : null} showAnchorGizmo={profile.mode === "rigid"} showSkeleton={showSkeleton} onReferenceStatus={running ? undefined : setStatus} />
              </div>
            </div>
            <aside className="space-y-3">
              <ProfileCard category={category} side={side} onSideChange={setSide} />
              <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
                <div className="mb-3 flex items-center justify-between"><h2 className="flex items-center gap-2 font-black"><Move3d className="h-4 w-4 text-violet-300" /> Ajuste visual</h2><button type="button" onClick={resetFit} className="flex items-center gap-1 rounded-lg border border-white/10 px-2 py-1 text-xs text-white/55"><RotateCcw className="h-3 w-3" /> Reiniciar</button></div>
                <ChoiceRow values={["Frente", "Lateral", "Espalda"]} selected={view} onSelect={(value) => setView(value as View)} />
                <div className="mt-3"><ChoiceRow values={["Slim", "Regular", "Oversize"]} selected={fit} onSelect={(value) => setFit(value as Fit)} /></div>
                <div className="mt-3"><ChoiceRow values={["T-Pose", "Idle", "Walk"]} selected={pose} onSelect={(value) => setPose(value as Pose)} /></div>
                ...
                <button type="button" onClick={() => setShowSkeleton((value) => !value)} className="mt-4 w-full rounded-xl border border-white/10 px-3 py-3 text-sm font-bold text-white/65">{showSkeleton ? "Ocultar esqueleto" : "Ver esqueleto"}</button>
              </div>
              <PrimaryButton onClick={() => setStep("rig")}>Confirmar ajuste <ArrowRight className="h-5 w-5" /></PrimaryButton>
            </aside>
          </section>
        ) : null}
        ...
        {step === "rig" && asset ? (
          <section className="mx-auto max-w-3xl">
            ...
            <div className="mt-4 rounded-3xl border border-white/10 bg-white/[0.035] p-5 sm:p-7">
              <div className="flex gap-3"><div className="rounded-2xl bg-violet-500/15 p-3 text-violet-300"><Sparkles className="h-6 w-6" /></div><div><h2 className="text-xl font-black">Rig inteligente por categoría</h2><p className="mt-1 text-sm leading-6 text-white/55">No tenés que elegir scripts técnicos. CLOUVA usa automáticamente el pipeline correcto.</p></div></div>
              <div className="mt-5 grid gap-2 sm:grid-cols-3"><InfoPill label="Modo" value={profile.mode === "deformable" ? "Prenda deformable" : "Accesorio rígido"} /><InfoPill label="Destino" value={profile.anchor} /><InfoPill label="Pruebas" value={profile.validationPoses.join(" · ")} /></div>
              {running || progress > 0 ? <div className="mt-5 rounded-2xl border border-white/10 bg-black/25 p-4"><div className="mb-2 flex justify-between gap-3 text-sm"><span className="text-white/65">{status}</span><strong>{Math.round(progress)}%</strong></div><div className="h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-gradient-to-r from-violet-600 to-fuchsia-400 transition-all" style={{ width: `${progress}%` }} /></div></div> : null}
              {error ? <div className="mt-4 rounded-2xl border border-red-400/35 bg-red-500/10 p-4 text-sm text-red-200">{error}</div> : null}
              <button type="button" disabled={running} onClick={() => void applySmartRig()} className="mt-5 flex w-full items-center justify-center gap-3 rounded-2xl bg-gradient-to-r from-violet-600 to-fuchsia-600 px-5 py-5 font-black shadow-2xl shadow-violet-950/50 disabled:opacity-60">{running ? <Loader2 className="h-5 w-5 animate-spin" /> : <WandSparkles className="h-5 w-5" />}{running ? "Blender está trabajando…" : profile.actionLabel}</button>
...
function PrimaryButton({ children, onClick }: { children: React.ReactNode; onClick: () => void }) {
  return <button type="button" onClick={onClick} className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-violet-600 px-5 py-4 font-black shadow-xl shadow-violet-950/40">{children}</button>;
}

function ChoiceRow({ values, selected, onSelect }: { values: string[]; selected: string; onSelect: (value: string) => void }) {
  return <div className="grid grid-cols-3 gap-2">{values.map((value) => <button key={value} type="button" onClick={() => onSelect(value)} className={`rounded-xl border px-2 py-3 text-xs font-bold sm:text-sm ${selected === value ? "border-violet-400 bg-violet-500/20" : "border-white/10 bg-black/20 text-white/55"}`}>{value}</button>)}</div>;
}
...
function ProfileCard({ category, side, onSideChange, expanded = false }: { category: ReferenceCategory; side: Side; onSideChange: (side: Side) => void; expanded?: boolean }) {
  const profile = resolveRigProfile(category);
  return <div className="rounded-2xl border border-violet-400/20 bg-gradient-to-br from-violet-600/12 to-white/[0.025] p-4 sm:p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-violet-300">Perfil automático</p><h2 className="mt-1 text-lg font-black">{profile.label}</h2></div><span className={`rounded-full px-3 py-1 text-[10px] font-black uppercase ${profile.mode === "deformable" ? "bg-cyan-400/10 text-cyan-200" : "bg-amber-400/10 text-amber-200"}`}>{profile.mode === "deformable" ? "Deformable" : "Rígido"}</span></div><p className="mt-3 text-sm leading-6 text-white/60">{profile.summary}</p>{expanded ? <div className="mt-4 grid gap-3 sm:grid-cols-2"><InfoPill label="Huesos" value={profile.requiredBones.join(" · ")} /><InfoPill label="Regiones" value={profile.bodyRegions.join(" · ")} /></div> : null}{profile.sided ? <div className="mt-4 grid grid-cols-2 gap-2"><button type="button" onClick={() => onSideChange("left")} className={`rounded-xl border px-3 py-3 text-sm font-bold ${side === "left" ? "border-violet-400 bg-violet-500/20" : "border-white/10 text-white/50"}`}>Izquierda</button><button type="button" onClick={() => onSideChange("right")} className={`rounded-xl border px-3 py-3 text-sm font-bold ${side === "right" ? "border-violet-400 bg-violet-500/20" : "border-white/10 text-white/50"}`}>Derecha</button></div> : null}</div>;
}

function Slider({ label, value, min, max, onChange }: { label: string; value: number; min: number; max: number; onChange: (value: number) => void }) {
  return <label className="block"><span className="mb-2 flex justify-between text-xs font-bold text-white/55"><span>{label}</span><strong className="text-white">{Math.round(value)}</strong></span><input type="range" min={min} max={max} value={value} onChange={(event) => onChange(Number(event.target.value))} className="w-full accent-violet-500" /></label>;
}

function InfoPill({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl border border-white/10 bg-black/20 p-3"><p className="text-[10px] font-black uppercase tracking-[0.12em] text-white/35">{label}</p><p className="mt-1 text-sm font-bold text-white/75">{value}</p></div>;
}

function EmptyResult({ onBack }: { onBack: () => void }) {
  return <div className="rounded-3xl border border-white/10 bg-white/[0.035] p-8 text-center"><ShieldCheck className="mx-auto h-10 w-10 text-violet-300" /><h2 className="mt-4 text-xl font-black">Todavía no hay resultado</h2><button type="button" onClick={onBack} className="mt-4 rounded-2xl bg-violet-600 px-5 py-3 font-black">Ir al rig</button></div>;
}
