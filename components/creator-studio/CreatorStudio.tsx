...
const rawPipeline = [
  "Subiendo GLB existente", "Importando en Blender", "Alineando con clouva_base_v1", "Aplicando ajuste de superficie",
  "Transfiriendo Vertex Groups", "Transfiriendo pesos", "Vinculando Armature", "Normalizando influencias",
  "Prueba T-Pose", "Prueba Idle", "Prueba Walk", "Guardando GLB riggeado en Supabase",
];

const templatePipeline = [
  "Cargando plantilla existente", "Importando en Blender", "Conservando topología", "Conservando Vertex Groups",
  "Conservando pesos", "Vinculando al Armature oficial", "Aplicando deformaciones", "Corrigiendo clipping",
  "Prueba T-Pose", "Prueba Idle", "Prueba Walk", "Exportando GLB validado",
];
...
export function CreatorStudio() {
  ...
  const [message, setMessage] = useState("Listo para usar los GLB guardados en Supabase");
  ...
  const currentStep = useMemo(() => Math.min(Math.floor((progress / 100) * pipeline.length), pipeline.length - 1), [pipeline.length, progress]);
  const currentPreviewSettings = useMemo(() => ({ fit, pose, view, adjustments }), [adjustments, fit, pose, view]);
    ...
    if (doneStates.has(normalizedStatus) || data.resultUrl) {
      ...
      if (data.resultUrl && activeAsset) {
        try {
          ...
          setMessage(`El rig terminó y se puede descargar, pero no se guardó en la biblioteca: ${error instanceof Error ? error.message : "error desconocido"}`);
        }
      } else {
  ...
  useEffect(() => {
    ...
    try {
      ...
      })().catch((error) => setMessage(error instanceof Error ? error.message : "No se pudo recuperar el Auto Rig."));
    } catch {
      saveActiveJob(null);
    }
  }, [checkJob]);
  ...
  function resetProject() {
    ...
    setReferenceModelUrl((current) => { if (current) URL.revokeObjectURL(current); return null; });
    setResultUrl(null); setProgress(0); setJobId(null); setJobStatus("idle"); setJobStage(null); setRunning(false);
    saveActiveJob(null); resetPreview(); setTab("create"); setMessage("Nuevo proyecto creado");
  }
  ...
  async function rigReference() {
    ...
    setRunning(true); setTab("process"); setProgress(1); setJobStatus("uploading");
    setJobStage(isTemplate ? "Cargando plantilla existente" : "Subiendo GLB existente");
    setMessage(isTemplate ? "Enviando la plantilla al Blender Worker sin recalcular pesos…" : "Enviando el GLB existente al Blender Worker para transferir el rig…");
    ...
    try {
      ...
      if (data.resultUrl) {
        await applyJobStatus({ ...data, status: "completed" }, String(data.jobId ?? "direct"), referenceAsset, currentPreviewSettings);
        return;
      }
    ...
    <main style={{ minHeight: "100dvh", background: "radial-gradient(circle at 20% 0%,#271045 0,#0b0711 38%,#050507 100%)", color: "white", padding: 22, fontFamily: "Inter,system-ui,sans-serif" }}>
      <div style={{ maxWidth: 1440, margin: "0 auto" }}>
        <header style={{ display: "flex", justifyContent: "space-between", gap: 18, alignItems: "center", marginBottom: 22, flexWrap: "wrap" }}>
          <div><div style={{ display: "flex", alignItems: "center", gap: 10, color: "#c4a7ff", fontWeight: 800 }}><WandSparkles size={18}/> CLOUVA</div><h1 style={{ margin: "5px 0 3px", fontSize: "clamp(28px,5vw,52px)" }}>Creator Studio</h1><p style={{ margin: 0, color: "#aaa3b5" }}>GLB de Supabase → ajustar → riggear o conservar rig → validar → Marketplace</p></div>
        ...
        <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12, marginBottom: 18 }}>
          {[{label:"Referencias",value:referenceAsset?"1":"0",icon:<Box/>},{label:"Procesando",value:running?"1":"0",icon:<Activity/>},{label:"Plantillas",value:referenceAsset?.isTemplate?"1":"0",icon:<CheckCircle2/>},{label:"Errores",value:errorCount,icon:<CircleDashed/>}].map((item)=><div key={item.label} style={card}><div style={{color:"#bda2ff"}}>{item.icon}</div><strong style={{fontSize:26}}>{item.value}</strong><span style={{color:"#9e97a8"}}>{item.label}</span></div>)}
        </section>

        <nav style={{ display: "flex", gap: 8, overflowX: "auto", marginBottom: 16 }}>
          {(["create","preview","process","publish"] as const).map((item,index)=><button key={item} onClick={()=>setTab(item)} style={{...tabButton,...(tab===item?activeTab:{})}}>{index+1}. {item==="create"?"Biblioteca":item==="preview"?"Probar y riggear":item==="process"?"Blender Worker":"Resultado"}</button>)}
        </nav>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1.5fr) minmax(280px,.7fr)", gap: 16 }} className="creator-grid">
          <section style={{ ...panel, minHeight: 620 }}>
            {tab === "create" && <div>
              <h2 style={title}><ImagePlus/> Biblioteca de GLB existentes</h2>
              ...
              <ReferenceAssetLibrary key={libraryVersion} selectedAssetId={referenceAsset?.id ?? null} onCategoryChange={(value:ReferenceCategory)=>setCategory(value)} onSelect={(asset,url)=>{setReferenceAsset(asset);setReferenceModelUrl((current)=>{if(current)URL.revokeObjectURL(current);return url;});if(asset)setMessage(asset.isTemplate?`${asset.name} es una plantilla lista para deformar.`:`${asset.name} está listo para probar y Auto Rig.`);}}/>
            ...
            {tab === "preview" && <div>
              ...
              <div style={notice}>{templateMode?"Este GLB conservará sus pesos, Vertex Groups, topología y rig. Blender solo aplicará ajustes y validaciones.":"Blender transferirá pesos y Vertex Groups desde clouva_base_v1, agregará el Armature y validará que la prenda acompañe las animaciones."}</div>
              {!templateMode&&<button disabled={promoting||!referenceAsset} onClick={()=>void promoteSelectedAsset()} style={{...secondaryButton,width:"100%",justifyContent:"center",marginTop:10,opacity:referenceAsset&&!promoting?1:.5}}><CheckCircle2 size={17}/> {promoting?"Guardando plantilla…":"Este GLB ya tiene rig: usar como plantilla base"}</button>}
              <button disabled={running||!referenceAsset} onClick={()=>void rigReference()} style={{...primaryButton,width:"100%",justifyContent:"center",marginTop:10,opacity:referenceAsset&&!running?1:.5}}><Settings2/> {running?"Procesando…":templateMode?"Procesar desde plantilla":"Auto Rig: transferir rig del avatar"}</button>
            </div>}

            {tab === "process" && <div><h2 style={title}><Settings2/> {templateMode?"Procesando plantilla":"Auto Rig con Blender Worker"}</h2><div style={{marginBottom:12,color:"#c9bfd3"}}>{jobId?`Trabajo: ${jobId}`:"Preparando trabajo…"}</div><div style={{height:10,background:"#211a29",borderRadius:99,overflow:"hidden",marginBottom:18}}><div style={{width:`${progress}%`,height:"100%",background:"linear-gradient(90deg,#7c3aed,#d8b4fe)",transition:"width .35s"}}/></div>{jobStage&&<div style={notice}>Etapa real: {jobStage}</div>}<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:8,marginTop:14}}>{pipeline.map((step,index)=><div key={step} style={{display:"flex",alignItems:"center",gap:10,padding:11,borderRadius:12,background:index<currentStep?"#17251e":index===currentStep?"#261b35":"#110e15",color:index<=currentStep?"white":"#736d7b"}}>{index<currentStep?<CheckCircle2 color="#65d894" size={18}/>:<CircleDashed size={18}/>} {step}</div>)}</div><div style={{...notice,marginTop:16}}>Podés refrescar o cerrar la página. CLOUVA guarda el jobId y retoma el seguimiento automáticamente.</div></div>}

            {tab === "publish" && <div><h2 style={title}><Download/> Resultado de Blender</h2><div style={failedStates.has(jobStatus)||jobStatus==="error"?errorBox:successBox}><CheckCircle2 size={42}/><div><strong>{resultUrl?"GLB procesado":failedStates.has(jobStatus)||jobStatus==="error"?"Proceso con error":"Proceso en espera"}</strong><p style={{margin:"5px 0 0",color:"#b8c9bd"}}>{message}</p>{jobId&&<small style={{display:"block",marginTop:8}}>Job: {jobId}</small>}</div></div>{resultUrl?<a href={resultUrl} style={{...primaryButton,textDecoration:"none"}}><Download size={16}/> Descargar GLB riggeado</a>:<button onClick={()=>setTab(jobId?"process":"preview")} style={primaryButton}><Play size={16}/> {jobId?"Ver seguimiento":"Volver al ajuste"}</button>}</div>}
          </section>

          <aside style={panel}><h3 style={{marginTop:0}}>Estrategia automática</h3>{(templateMode?["Preservar rig","Preservar pesos","Preservar topología","Aplicar deformaciones","Corregir clipping","Pruebas T-Pose / Idle / Walk"]:["Auto Fix","Transferir pesos","Transferir Vertex Groups","Vincular Armature","Normalizar pesos","Pruebas T-Pose / Idle / Walk"]).map((setting)=><label key={setting} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"11px 0",borderBottom:"1px solid #211b29"}}><span>{setting}</span><input type="checkbox" checked readOnly/></label>)}<h3>Estado</h3><div style={{padding:14,background:"#100c14",borderRadius:14,color:"#c9bfd3",lineHeight:1.5}}>{message}</div><div style={{marginTop:14,color:"#91899b",fontSize:13}}>Progreso real: {progress}% · Estado: {jobStatus}</div>{jobId&&<Info title="Job activo" value={jobId}/>}<Info title="Asset activo" value={referenceAsset?.name??"Ninguno"}/><Info title="Modo" value={templateMode?"Plantilla Base-Mesh":"Auto Rig por transferencia"}/><Info title="Anclaje detectado" value={anchorByCategory[category]}/></aside>
        </div>
      </div>
