...
export function CreatorStudio() {
  ...
  function resetProject() {
    setPrompt(""); setImageName(null);
    setImagePreviewUrl((current) => { if (current) URL.revokeObjectURL(current); return null; });
    setReferenceAsset(null); setReferenceModelUrl(null); setProgress(0); setGenerated(false);
    resetPreview(); setTab("create"); setMessage("Nuevo proyecto creado");
  }

  async function generateModel() {
    setRunning(true); setProgress(8); setMessage("Enviando solicitud aprobada a Meshy…");
    try {
      const response = await fetch("/api/creator-studio/meshy", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          prompt, category, quality: "high", polycount: 30000, textureResolution: 2048,
          previewSettings: { fit, pose, view, adjustments },
          referenceAsset: referenceAsset ? {
            name: referenceAsset.name, category: referenceAsset.category, sourceUrl: referenceAsset.sourceUrl,
            license: referenceAsset.license, author: referenceAsset.author, fileName: referenceAsset.fileName,
          } : null,
        }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "No se pudo iniciar Meshy");
      setGenerated(true); setProgress(35);
      setMessage(data.mock ? "Referencia aprobada. Falta configurar MESHY_API_KEY para generar el modelo real." : `Meshy iniciado: ${data.taskId}`);
      setTab("viewer");
  ...
  async function processInBlender() {
    setRunning(true); setTab("process"); setMessage("Iniciando Blender Worker…");
    ...
    try {
      const response = await fetch("/api/creator-studio/blender", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ category, rig: "clouva_base_v1", autoFix: true, autoWeight: true, autoExport: true,
          targetPolycount: 25000, maxFileSizeMb: 18, textureResolution: 2048, formats: ["glb", "fbx", "obj", "blend"],
          previewSettings: { fit, pose, view, adjustments }, referenceAssetName: referenceAsset?.name ?? null }),
      });
      const data = await response.json();
    ...
    <main style={{ minHeight: "100dvh", background: "radial-gradient(circle at 20% 0%, #271045 0, #0b0711 38%, #050507 100%)", color: "white", padding: 22, fontFamily: "Inter, system-ui, sans-serif" }}>
      <div style={{ maxWidth: 1440, margin: "0 auto" }}>
        <header style={{ display: "flex", justifyContent: "space-between", gap: 18, alignItems: "center", marginBottom: 22, flexWrap: "wrap" }}>
          <div><div style={{ display: "flex", alignItems: "center", gap: 10, color: "#c4a7ff", fontWeight: 800, letterSpacing: 1.5 }}><WandSparkles size={18}/> CLOUVA</div>
            <h1 style={{ margin: "5px 0 3px", fontSize: "clamp(28px, 5vw, 52px)", lineHeight: 1 }}>Creator Studio</h1>
            <p style={{ margin: 0, color: "#aaa3b5" }}>GLB real de referencia → Vista previa sobre avatar → Meshy → Blender → Marketplace</p></div>
        ...
        <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12, marginBottom: 18 }}>
          {[{label:"Referencias",value:referenceAsset?"1":"0",icon:<Box/>},{label:"Procesando",value:running?"1":"0",icon:<Activity/>},{label:"Listos",value:generated?"1":"0",icon:<CheckCircle2/>},{label:"Errores",value:"0",icon:<CircleDashed/>}].map((item) => <div key={item.label} style={card}><div style={{ color: "#bda2ff" }}>{item.icon}</div><strong style={{ fontSize: 26 }}>{item.value}</strong><span style={{ color: "#9e97a8" }}>{item.label}</span></div>)}
        </section>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1.5fr) minmax(280px, .7fr)", gap: 16 }} className="creator-grid">
          <section style={{ ...panel, minHeight: 620 }}>
            {tab === "create" && <div>
              <h2 style={title}><ImagePlus/> Referencias del modelo</h2>
              <label style={label}>Imagen o diseño que Meshy debe recrear</label>
              <label style={{ ...dropzone, borderColor: imageName ? "#8b5cf6" : "#40364b" }}><Upload/><span>{imageName ?? "Tocá para subir PNG, JPG o WEBP"}</span>
                <input hidden type="file" accept="image/*" onChange={(event) => { const file = event.target.files?.[0]; setImageName(file?.name ?? null); setImagePreviewUrl((current) => { if (current) URL.revokeObjectURL(current); return file ? URL.createObjectURL(file) : null; }); }}/></label>
              <label style={label}>Prompt</label>
              <textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="Ejemplo: zapatilla urbana alta, cuero negro gastado, suela blanca…" style={{ ...input, minHeight: 110, resize: "vertical" }}/>
              ...
              <ReferenceAssetLibrary selectedAssetId={referenceAsset?.id ?? null} onCategoryChange={(value: ReferenceCategory) => setCategory(value)} onSelect={(asset, url) => { setReferenceAsset(asset); setReferenceModelUrl(url); if (asset) setMessage(`${asset.name} listo para probar sobre el avatar.`); }}/>
            ...
            {tab === "preview" && <div>
              <h2 style={title}><UserRound/> Probar referencia real en mi avatar</h2>
              <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1.2fr) minmax(250px,.8fr)", gap: 16 }} className="preview-grid">
                <div><div style={{ ...smartViewer, background }}>
                  <SmartTryOnViewer category={category} fit={fit} pose={pose} view={view} background={background} showBody={showBody} garmentOnly={garmentOnly} adjustments={{ ...adjustments, rotation: adjustments.rotation + rotation, scale: adjustments.scale * zoom }} imageUrl={imagePreviewUrl} referenceModelUrl={referenceModelUrl} onReferenceStatus={setMessage}/>
                  <div style={viewerBadge}>{referenceAsset ? `${referenceAsset.name} · GLB real · 0 créditos` : "Sin GLB de referencia"}</div></div>
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 }}><button onClick={() => setRotation((value) => value - 45)} style={toolButton}>↶ Rotar</button><button onClick={() => setRotation((value) => value + 45)} style={toolButton}>Rotar ↷</button><button onClick={() => setZoom((value) => Math.min(2, value + .1))} style={toolButton}>Zoom +</button><button onClick={() => setZoom((value) => Math.max(.35, value - .1))} style={toolButton}>Zoom −</button><button onClick={() => setShowBody((value) => !value)} style={toolButton}>{showBody ? "Ocultar cuerpo" : "Mostrar cuerpo"}</button><button onClick={() => setGarmentOnly((value) => !value)} style={toolButton}>{garmentOnly ? "Mostrar avatar" : "Solo referencia"}</button><button onClick={resetPreview} style={toolButton}><RotateCcw size={15}/> Reiniciar</button></div>
                </div>
                <div><div style={formGrid}><Field label="Ajuste"><select value={fit} onChange={(event) => setFit(event.target.value as Fit)} style={input}><option>Slim</option><option>Regular</option><option>Oversize</option></select></Field><Field label="Fondo"><input type="color" value={background} onChange={(event) => setBackground(event.target.value)} style={{ ...input, height: 46, padding: 5 }}/></Field></div>
                  <Range label="Escala" value={adjustments.scale} min={25} max={300} onChange={(value) => updateAdjustment("scale", value)}/><Range label="Largo / altura" value={adjustments.length} min={35} max={240} onChange={(value) => updateAdjustment("length", value)}/><Range label="Ancho" value={adjustments.width} min={35} max={240} onChange={(value) => updateAdjustment("width", value)}/><Range label="Posición X" value={adjustments.x} min={-150} max={150} onChange={(value) => updateAdjustment("x", value)}/><Range label="Posición Y" value={adjustments.y} min={-150} max={150} onChange={(value) => updateAdjustment("y", value)}/><Range label="Rotación" value={adjustments.rotation} min={-180} max={180} onChange={(value) => updateAdjustment("rotation", value)}/><Range label="Altura" value={adjustments.height} min={-100} max={100} onChange={(value) => updateAdjustment("height", value)}/><Range label="Profundidad" value={adjustments.distance} min={-40} max={60} onChange={(value) => updateAdjustment("distance", value)}/>
                </div>
              </div>
              <div style={{ marginTop: 14, padding: 12, borderRadius: 12, background: "#16101c", border: "1px solid #3a2c46", color: "#cfc3db" }}>Este visor usa el GLB real que subiste. El encaje es una referencia visual; Blender Worker hace el ajuste, rig y pesos definitivos.</div>
              <button disabled={running || !referenceAsset || (!prompt && !imageName)} onClick={generateModel} style={{ ...primaryButton, width: "100%", justifyContent: "center", marginTop: 14, opacity: referenceAsset && (prompt || imageName) ? 1 : .5 }}><Sparkles/> {running ? "Generando…" : "Usar esta referencia y generar con Meshy"}</button>
            </div>}

            {tab === "viewer" && <div style={{ height: "100%" }}><h2 style={title}><Eye/> Comparar referencia ↔ modelo generado</h2><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 14 }}><div style={viewer}><div style={{ textAlign: "center" }}><UserRound size={58} strokeWidth={1}/><h3>{referenceAsset?.name ?? "Referencia aprobada"}</h3><p style={{ color: "#978fa3" }}>{category} · {fit} · {anchorByCategory[category]}</p></div></div><div style={viewer}><div style={{ textAlign: "center" }}><Layers3 size={58} strokeWidth={1}/><h3>Modelo generado por Meshy</h3><p style={{ color: "#978fa3" }}>Se reemplaza automáticamente cuando Meshy devuelve el GLB real.</p></div></div></div><button onClick={processInBlender} disabled={running} style={{ ...primaryButton, width: "100%", justifyContent: "center", marginTop: 14 }}><Play/> Enviar a Blender</button></div>}

            {tab === "process" && <div><h2 style={title}><Settings2/> Monitor Blender Worker</h2><div style={{ height: 10, background: "#211a29", borderRadius: 99, overflow: "hidden", marginBottom: 18 }}><div style={{ width: `${progress}%`, height: "100%", background: "linear-gradient(90deg,#7c3aed,#d8b4fe)", transition: "width .2s" }}/></div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 8 }}>{pipeline.map((step, index) => <div key={step} style={{ display: "flex", alignItems: "center", gap: 10, padding: 11, borderRadius: 12, background: index < currentStep ? "#17251e" : index === currentStep ? "#261b35" : "#110e15", color: index <= currentStep ? "white" : "#736d7b" }}>{index < currentStep ? <CheckCircle2 color="#65d894" size={18}/> : <CircleDashed size={18}/>} {step}</div>)}</div></div>}

            {tab === "publish" && <div><h2 style={title}><Download/> Publicar y exportar</h2><div style={successBox}><CheckCircle2 size={42}/><div><strong>Modelo preparado para CLOUVA</strong><p style={{ margin: "5px 0 0", color: "#b8c9bd" }}>Compatible con clouva_base_v1, body masks, slots y animaciones.</p></div></div><div style={{ display: "flex", gap: 9, flexWrap: "wrap" }}>{["GLB","FBX","OBJ","BLEND","PNG","Render 360°","GIF"].map((format) => <button key={format} style={toolButton}><Download size={15}/>{format}</button>)}</div><button style={{ ...primaryButton, width: "100%", justifyContent: "center", marginTop: 18 }}>Publicar en Marketplace</button></div>}
          </section>

          <aside style={panel}><h3 style={{ marginTop: 0 }}>Configuración automática</h3>{["Auto Fix","Auto Weight","Auto Rig","Auto Export","Shrinkwrap","Surface Deform","Transfer Vertex Groups","Generar LOD","Bake PBR","Comprimir texturas"].map((setting) => <label key={setting} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "11px 0", borderBottom: "1px solid #211b29" }}><span>{setting}</span><input type="checkbox" defaultChecked/></label>)}<h3>Estado</h3><div style={{ padding: 14, background: "#100c14", borderRadius: 14, color: "#c9bfd3", lineHeight: 1.5 }}>{message}</div><div style={{ marginTop: 14, color: "#91899b", fontSize: 13 }}>Progreso general: {progress}%</div><div style={{ marginTop: 14, padding: 12, borderRadius: 12, border: "1px solid #30243a", background: "#0e0a12" }}><strong style={{ display: "block", marginBottom: 6 }}>Referencia activa</strong><span style={{ color: "#bda2ff" }}>{referenceAsset?.name ?? "Ninguna"}</span></div><div style={{ marginTop: 10, padding: 12, borderRadius: 12, border: "1px solid #30243a", background: "#0e0a12" }}><strong style={{ display: "block", marginBottom: 6 }}>Anclaje detectado</strong><span style={{ color: "#bda2ff" }}>{anchorByCategory[category]}</span></div></aside>
        </div>
      </div>
