...
function avatarStatusLabel(status: string) {
    ...
    <section className="mt-8 rounded-[2rem] border border-white/10 bg-black/25 p-5 sm:p-6">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        {avatars.map((avatar) => {
          ...
          const hasStoredModel = Boolean(avatar.model_url) && (ready || avatar.status === "pending_analysis");
          return (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className={`mt-1 text-xs ${avatar.status === "pending_analysis" ? "text-amber-200" : "text-white/45"}`}>
                      {avatarStatusLabel(avatar.status)}
                    </p>
                  </div>
