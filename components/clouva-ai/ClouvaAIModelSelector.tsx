...
const COOKIE_NAME = "clouva_ai_model";
const STORAGE_KEY = "clouva.ai.model.v2";
const LEGACY_STORAGE_KEY = "clouva.ai.gemini-model.v1";
...
export function ClouvaAIModelSelector() {
  const [models, setModels] = useState<ModelOption[]>([]);
  const [selected, setSelected] = useState("");
  const [defaultModel, setDefaultModel] = useState("");
  const [provider, setProvider] = useState("ClouAI");
  ...
  async function loadModels() {
    ...
    try {
      ...
      const configuredDefault = payload.defaultModel ?? available[0]?.id ?? "";
      ...
      const next = saved && available.some((model) => model.id === saved)
        ? saved
      ...
      setProvider(payload.provider ?? "ClouAI");
      setSelected(next);
      if (next) saveModel(next);
    } catch (caught) {
  ...
  const current = useMemo(() => models.find((model) => model.id === selected), [models, selected]);
  ...
  return (
    <div className="group relative flex min-w-0 items-center gap-2 rounded-xl border border-white/10 bg-white/[0.045] p-1.5 transition hover:border-violet-400/35">
      ...
      <div className="relative min-w-0 flex-1">
        ...
        <select
          value={selected}
          onChange={(event) => { setSelected(event.target.value); saveModel(event.target.value); }}
        ...
        >
          {models.length === 0 ? (
            <option value={defaultModel}>{defaultModel || "ClouAI"}</option>
          ) : models.map((model) => (
            <option key={model.id} value={model.id} className="bg-zinc-950">{model.name}</option>
          ))}
        </select>
