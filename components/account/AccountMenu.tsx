...
export function AccountMenu({ variant = "nav", triggerClassName = "" }: AccountMenuProps) {
  ...
  useEffect(() => {
    if (openSwitch) setAccounts(getAccounts());
  }, [openSwitch]);

  useEffect(() => {
    if (typeof window === "undefined" || !user) return;
    if (new URLSearchParams(window.location.search).get("openAccountSwitcher") === "1") setOpenSwitch(true);
  }, [user]);
  ...
  return (
    <div ref={rootRef} className={`${styles.root} ${variant === "home" ? styles.homeRoot : ""}`}>
      <button
        ref={triggerRef}
        type="button"
        className={`${styles.trigger} ${variant === "home" ? styles.homeTrigger : ""} ${triggerClassName}`}
        ...
        <section id={menuId} role="menu" aria-label="Tu cuenta CLOUVA" className={styles.popover}>
          ...
          <div className={styles.primaryLinks}>
            ...
            {canAdmin ? (
              ...
              <div role="menuitem" className={`${styles.menuItem} ${styles.disabled}`} aria-disabled="true">
                <span className={styles.itemIcon}><Boxes size={17} /></span>
                <span><b>Mi Avatar 3D</b><small>Próximamente</small></span>
              </div>
            )}
          ...
          <div className={styles.secondaryLinks}>
            ...
            <button type="button" role="menuitem" className={styles.menuItem} onClick={() => { setOpenMenu(false); setOpenSwitch(true); }}>
              <span className={styles.itemIcon}><Repeat2 size={16} /></span><span><b>Cambiar cuenta</b></span>
            </button>
            <button
              type="button"
              role="menuitem"
            ...
            >
              <span className={styles.itemIcon}><LogOut size={16} /></span><span><b>Cerrar sesión</b></span>
            </button>
          </div>
        </section>
      ) : null}
          ...
          <section className={styles.switchDialog}>
            ...
            <div className={styles.accountList}>
              {accounts.length ? accounts.map((account) => (
                <button key={account.id} type="button" onClick={() => { setOpenSwitch(false); void switchAccount(account.id); }}>
                  <span>{account.display_name.charAt(0).toUpperCase()}</span>
                  <div><b>{account.display_name}</b><small>{account.email}</small></div>
                </button>
              )) : <p>No hay otras cuentas guardadas en este dispositivo.</p>}
            </div>
