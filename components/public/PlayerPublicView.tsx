      ...
      <section className="relative isolate overflow-hidden border-b border-white/10">
        ...
        <div className="mx-auto grid min-h-[36rem] max-w-7xl gap-8 px-4 py-9 sm:px-6 lg:grid-cols-[minmax(0,1fr)_280px] lg:items-center">
          <div>
            ...
            <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
              <div className="relative shrink-0">
                {player.profile_image_url ? (
                  ...
                  <div className="grid h-28 w-28 place-items-center rounded-full border-2 border-[color:var(--public-accent)]/60 bg-[color:var(--public-accent)]/20 text-4xl font-black sm:h-36 sm:w-36">
                    {player.display_name.charAt(0)}
                  </div>
                )}
                {player.is_verified ? (
                  <span className="absolute bottom-1 right-1 grid h-8 w-8 place-items-center rounded-full border-2 border-[#07060b] bg-[color:var(--public-accent)] text-white">
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  {isVip ? (
                    <span className="inline-flex items-center gap-1.5 rounded-full border border-[color:var(--public-accent)]/35 bg-[color:var(--public-accent)]/15 px-3 py-1.5 text-xs font-semibold text-[color:var(--public-accent)]">
                      <Crown size={14} /> VIP
                    </span>
                  ) : null}
                </div>
            ...
            {primaryStudio ? (
              <Link href={`/studios/${primaryStudio.slug}`} className={`mt-6 inline-flex items-center gap-3 border border-white/12 bg-black/35 px-4 py-3 backdrop-blur transition hover:border-[color:var(--public-accent)]/45 ${radiusClass}`}>
                ...
                <ArrowRight size={15} className="ml-2 text-[color:var(--public-accent)]" />
              </Link>
            ) : null}
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              {player.spotify_profile_url ? (
                <a href="#musica" className="inline-flex items-center gap-2 rounded-full bg-[color:var(--public-accent)] px-5 py-2.5 text-sm font-semibold transition hover:opacity-90">
      ...
      <section id="presentacion" className="mx-auto grid max-w-7xl gap-4 px-4 py-6 sm:px-6 lg:grid-cols-[minmax(0,.9fr)_minmax(0,1.4fr)_280px]">
        ...
        <section className={`border border-white/10 bg-white/[0.025] p-5 ${radiusClass}`}>
          ...
          {featuredMedia.length ? (
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{featuredMedia.map((item) => <MediaCard key={item.id} item={item} radiusClass={radiusClass} />)}</div>
          ) : (
            <div className={`grid min-h-44 place-items-center border border-dashed border-white/10 text-center text-xs text-white/35 ${radiusClass}`}>
        ...
        <aside className={`border border-white/10 bg-white/[0.025] p-5 ${radiusClass}`}>
          <div className="flex items-center justify-between gap-3">
            ...
            <Building2 size={16} className="text-[color:var(--public-accent)]" />
          </div>
          {affiliations.length ? (
